import{d as J,r as dt,g as P,e as pt,f as ct,h as ut,i as mt,j as ft,k as ht,l as bt,m as vt,n as gt,o as xt,p as yt,q as wt}from"./dungeon-CrrX07Lq.js";import{p as $t}from"./gamepack-D3b60uQv.js";import{C as R}from"./tables-3KScslBT.js";import{T as kt}from"./tables-doc-Clxj5t_C.js";import{f as V,p as _t,a as Lt,g as K,b as A,u as St,E as Tt,s as jt,o as Mt,t as Pt,d as Z}from"./tuner-core-jthNyDov.js";import{r as zt,h as Ot}from"./stale-card-CdbJ7s3s.js";import{o as Ct}from"./llm-_hC_o_Th.js";import{h as Ht,t as o}from"./ui-confirm-B1a0wtbi.js";import"./play-input-BYi92GtF.js";import"./status-CoghzNI4.js";import"./three.module-C7yXn3za.js";import"./pack-handoff-client-C-AMTnth.js";const Et=`<!-- ============================================================
...
<div id="app">
  <div id="side">
    <div id="about" style="display:flex;flex-direction:column;gap:9px">
      ...
      <div class="sub"><span data-i18n="tool.tuner.intro">保存は下書きとして・採用は⭐で確定版にします。右の下見で「このつまみだと実際どんな盤が出るか」を確かめられます（つまみは全部この世界の設定です）。</span><a href="./studio.html?panel=forge" style="color:#8fb7ff" data-i18n="tool.tuner.linkForge">部屋と街</a>・<a href="./studio.html?panel=smith" style="color:#8fb7ff" data-i18n="tool.tuner.linkSmith">敵と道具</a>・<a href="./dungeon.html" style="color:#8fb7ff" data-i18n="tool.tuner.linkGame">実機</a></div>
  ...
  <div id="mid">
    <div id="stage">
      <div id="ctrl">
        ...
        <span class="lbl" data-i18n="tool.tuner.seedBase">配置ちがい</span>
    ...
    <div id="foot"><span id="hint" data-i18n="tool.common.savenote">保存しても確定版は変わりません。右の一覧に下書きが並び、⭐採用したものだけが確定版になります。</span><span id="perf"></span><span id="msg"></span></div>
  </div>
  <div id="loop">
    ...
    <button class="big" id="save" type="button" data-i18n="tool.tuner.save">💾 下書きとして保存</button>
    <h2 data-i18n="tool.tuner.futureHead">⭐ 下書き</h2>
    <div id="proposals"></div>
    <h2 data-i18n="tool.tuner.truthHead">📜 確定版（この世界の調律）</h2>
    <div id="truth"></div>
  </div>
</div>
`,c=g=>g.replace(/[&<>"]/g,T=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[T]),Y=()=>new Date().toISOString().replace(/\.\d+Z$/,"Z");try{const g=Object.assign({"../../tenants/masaru/comduct/prefabs/deep-throne.json":yt,"../../tenants/masaru/comduct/prefabs/interior-craft.json":xt,"../../tenants/masaru/comduct/prefabs/interior-equip.json":gt,"../../tenants/masaru/comduct/prefabs/interior-heal.json":vt,"../../tenants/masaru/comduct/prefabs/interior-recruit.json":bt,"../../tenants/masaru/comduct/prefabs/interior-shop.json":ht,"../../tenants/masaru/comduct/prefabs/manifest.json":ft,"../../tenants/masaru/comduct/prefabs/root-den.json":mt,"../../tenants/masaru/comduct/prefabs/scribe-alcove.json":ut,"../../tenants/masaru/comduct/prefabs/shrine-guardian.json":ct}),T=[];for(const w of Object.keys(g).sort())w.endsWith("manifest.json")||T.push($t(g[w].default??g[w]));wt(T)}catch(g){console.warn("truth プレハブの読込に失敗（プレハブ無しの束見で継続）",g)}const Ft={tuning:R.tuning,gen:R.genParams,gim:R.gimmicks},Kt={id:"tuner",async mount(g,T){const{wapi:w,postJson:C}=T.api;g.innerHTML=Et,Ht(g);const s=a=>g.querySelector(`#${a}`);function b(a,t=!1){const e=s("msg");e.textContent=a,e.style.color=t?"#ff7a6e":"#7fd17f"}let p=V(),x="curve",$="trap",u=3,O={};function H(){return Mt(p)}const D="cdTunerW2";function j(){try{sessionStorage.setItem(D,JSON.stringify({v:1,draft:p,tab:x,selGim:$,depth:u,seed:s("seedbase").value,count:s("count").value,label:s("savelabel").value}))}catch{}}function Q(){var a,t;try{const e=sessionStorage.getItem(D);if(!e)return!1;const n=JSON.parse(e);return n.v!==1||!((a=n.draft)!=null&&a.genParams)||!((t=n.draft)!=null&&t.gimmicks)?!1:(p=n.draft,x=n.tab??"curve",$=n.draft.gimmicks[n.selGim]?n.selGim:"trap",u=Math.max(1,Math.min(30,n.depth|0||3)),s("depth").value=String(u),s("depthv").textContent=`${u}F`,n.seed&&(s("seedbase").value=n.seed),n.count&&(s("count").value=n.count),n.label&&(s("savelabel").value=n.label),!0)}catch{return!1}}const f=(a,t,e,n,i=1,m="num")=>{const r=a.split(".").reduce((d,k)=>d==null?void 0:d[k],Ft);return`<input class="${m}${typeof r=="number"&&r!==t?" chg":""}" data-f="${a}" type="number" value="${t}" min="${e}" max="${n}" step="${i}" />`};function v(){const a=s("form");if(x==="curve"){const n=p.genParams,i=K(n,u),m=Object.keys(p.gimmicks).map(r=>{const l=p.gimmicks[r];return`<tr data-gim="${r}" class="${r===$?"sel":""}">
            ...
            <td class="sub">${Math.round(A(l,u)*100)}%</td>
          </tr>`}).join("");a.innerHTML=`
          ...
          <div class="sub">${c(o("tool.tuner.atDepth",{d:u,rooms:i.maxRooms,loops:i.loops}))}</div>
          ...
          <td><input class="wide" data-f="pack.${l}.units" type="text" value="${c(St(r.units))}" /></td>
          <td><button type="button" data-act="pack-del" data-i="${l}">✕</button></td></tr>`).join(""),i=p.materials.map((r,l)=>`<tr>
          ...
          <div class="tblwrap"><table class="grid"><thead><tr><th>id</th><th>${c(o("tool.tuner.colName"))}</th><th>min</th><th>value</th><th></th></tr></thead><tbody>${i}</tbody></table></div>
          <div class="kv"><button type="button" data-act="mat-add">＋ ${c(o("tool.tuner.addRow"))}</button></div>
          <h2>${c(o("tool.tuner.themeHead"))}</h2>
          <div class="tblwrap"><table class="grid"><thead><tr><th>${c(o("tool.tuner.colTheme"))}</th><th>col</th><th>mat</th><th>water</th><th>hazard</th></tr></thead><tbody>${m}</tbody></table></div>`;return}const t=p.tuning,e=(n,i)=>`<h2>${c(n)}</h2>`+i.map(([m,r,l,d])=>`<div class="kv"><span class="lbl" style="min-width:9em">${m}</span>${f(`tuning.${m}`,t[m],r,l,d)}</div>`).join("");a.innerHTML=e(o("tool.tuner.feelPos"),[["posBack",1,3,.1],["posSide",1,3,.1],["posHigh",1,3,.1]])+e(o("tool.tuner.feelWave"),[["waveFirstAfter",0,30,1],["waveGapBase",1,30,1],["waveGapJitter",0,10,1],["waveCountCap",1,12,1],["monsterCap",4,40,1]])+e(o("tool.tuner.feelGold"),[["assaultGoldBase",0,99,1],["assaultGoldPerDepth",0,30,1]])+e(o("tool.tuner.feelMat"),[["matDropBase",0,100,1],["matDropPerDepth",0,20,1],["themeMatN",0,10,1]])+e(o("tool.tuner.feelBreath"),[["breathLen",1,9,1],["breathCd",1,20,1]])+e(o("tool.tuner.feelLarge"),[["largePerFloor",0,4,1],["largePerFloorDeep",0,6,1],["largeDeepFrom",1,99,1]])}function B(){const a=g.querySelector("#curvechart");if(!a)return;const t=a.getContext("2d"),e=a.width,n=a.height,i=30;t.clearRect(0,0,e,n),t.strokeStyle="#2b3547",t.lineWidth=1;for(const d of[.25,.5,.75])t.beginPath(),t.moveTo(0,n*d),t.lineTo(e,n*d),t.stroke();const m=d=>(d-1)/(i-1)*(e-8)+4,r=d=>n-6-d*(n-14),l=(d,k,y)=>{t.strokeStyle=k,t.lineWidth=y,t.beginPath();for(let S=1;S<=i;S++){const U=r(A(p.gimmicks[d],S));S===1?t.moveTo(m(S),U):t.lineTo(m(S),U)}t.stroke()};for(const d of Object.keys(p.gimmicks))d!==$&&l(d,"rgba(139,150,168,0.28)",1);p.gimmicks[$]&&l($,"#ffb347",2),t.strokeStyle="#ffd166",t.lineWidth=1,t.setLineDash([3,3]),t.beginPath(),t.moveTo(m(u),0),t.lineTo(m(u),n),t.stroke(),t.setLineDash([]),t.fillStyle="#ffb347",t.font="10px ui-monospace",t.fillText(`${$} @${u}F = ${Math.round(A(p.gimmicks[$],u)*100)}%`,6,11)}const h=3;let E=0,F="";function X(a){let t=0;for(;P().floor.depth<a&&t++<a+30;){const e=P().actors.find(n=>n.kind==="hero");if(!e)return;e.pos={...P().floor.stairs},pt()}}function tt(a){const t=P(),e=t.floor;a.width=e.w*h,a.height=e.h*h;const n=a.getContext("2d");let i=0;for(let r=0;r<e.h;r++)for(let l=0;l<e.w;l++){const d=e.tiles[r][l];d.chest&&!d.chest.opened&&i++,n.fillStyle=Pt(d),n.fillRect(l*h,r*h,h,h)}n.fillStyle="#ffffff",n.fillRect(e.stairs.x*h,e.stairs.y*h,h,h),e.upstairs&&(n.fillStyle="#9aa7bd",n.fillRect(e.upstairs.x*h,e.upstairs.y*h,h,h));let m=0;for(const r of t.actors)r.alive&&(r.kind==="monster"?(m++,n.fillStyle="#ff5a4a"):r.kind==="hero"?n.fillStyle="#ffe9a8":n.fillStyle="#6ab0ff",n.fillRect(r.pos.x*h,r.pos.y*h,h,h));return{mon:m,chest:i}}async function M(){const a=++E;let t;try{t=H()}catch(d){s("lint").innerHTML=`<div class="err">✗ ${c(d.message.slice(0,800))}</div>`;return}O=t;const e=Object.keys(t).length;s("lint").innerHTML=`<div class="ok">✓ ${c(o("tool.tuner.lintOk",{n:e}))}</div>`,W(t),J(e?t:null);const n=jt(Number(s("seedbase").value)||0,Number(s("count").value)),i=s("grid");i.innerHTML=n.map((d,k)=>`<div class="thumb" data-i="${k}"><canvas></canvas><div class="cap">#${d} …</div></div>`).join("");const m=performance.now();for(let d=0;d<n.length;d++){if(a!==E)return;dt(n[d]),X(u);const k=i.children[d],y=tt(k.querySelector("canvas"));k.querySelector(".cap").textContent=`#${n[d]}  ${o("tool.tuner.capMon")}${y.mon} ${o("tool.tuner.capChest")}${y.chest}${P().floor.monsterHouse?" MH!":""}${P().floor.sealRoom?" 封":""}`,d%4===3&&await new Promise(S=>setTimeout(S))}const r=Math.round(performance.now()-m);F=o("tool.tuner.perf",{n:n.length,d:u,ms:r}),s("perf").textContent=F;const l=K(p.genParams,u);s("gensum").textContent=o("tool.tuner.atDepth",{d:u,rooms:l.maxRooms,loops:l.loops})}let N=0;function _(){clearTimeout(N),N=window.setTimeout(()=>{M()},180),j()}function W(a){const t=Object.keys(a);s("ovsum").textContent=t.length?o("tool.tuner.ovSome",{list:t.join(", ")}):o("tool.tuner.ovNone"),s("ovjson").textContent=t.length?JSON.stringify(a,null,1):"{}"}function I(){const a=[["#39455e",o("tool.tuner.lgWall")],["#141b2a",o("tool.tuner.lgFloor")],["#2a5f9e",o("tool.tuner.lgWater")],["#e8c15a",o("tool.tuner.lgChest")],["#c05a8a",o("tool.tuner.lgTrap")],["#a87b2a",o("tool.tuner.lgLocked")],["#6a4a8a",o("tool.tuner.lgSecret")],["#ff5a4a",o("tool.tuner.lgMon")],["#ffe9a8",o("tool.tuner.lgHero")],["#ffffff",o("tool.tuner.lgStairs")]];s("legend").innerHTML=a.map(([t,e])=>`<span><i style="background:${t}"></i>${c(e)}</span>`).join("")}async function L(){try{const[a,t]=await Promise.all([fetch(w("/api/tables/truth")).then(e=>e.json()),fetch(w("/api/tables/proposals")).then(e=>e.json())]);et(a.docs??[]),nt(t.proposals??[])}catch{s("truth").innerHTML=`<div class="live">${c(o("tool.tuner.noApi"))}</div>`,s("proposals").innerHTML=""}}function et(a){var i;const t=s("truth");if(!a.length){t.innerHTML=`<div class="live">${c(o("tool.tuner.truthEmpty"))}</div>`;return}const e=a[0],n=Object.keys(e.tables).length;t.innerHTML=`<div class="card"><b>${c(e.name)}</b><span class="id">${c(e.updatedAt)}・${n} ${c(o("tool.tuner.ovFields"))}</span>
        <div class="row"><button type="button" data-act="truth-load">${c(o("tool.tuner.load"))}</button></div></div>`,(i=t.querySelector('[data-act="truth-load"]'))==null||i.addEventListener("click",()=>{p=Z(e.tables),v(),_(),b(o("tool.tuner.loaded",{name:e.name}))})}function nt(a){zt(s("proposals"),a,{tag:()=>o("tool.tuner.capTag"),onView:t=>{G(t)},onPromote:t=>{at(t)},onArchive:t=>{ot(t)}})}async function G(a){var t;try{const n=(t=(await(await fetch(w(`/api/tables/proposal?id=${encodeURIComponent(a)}`))).json()).docs)==null?void 0:t[0];if(!n)throw new Error("proposal is empty");p=Z(n.tables),v(),_(),b(o("tool.tuner.loaded",{name:n.name}))}catch(e){b(o("tool.common.loadPropFail",{msg:e.message}),!0)}}async function at(a){try{await C(w("/api/tables/promote"),{proposalId:a}),b(o("tool.tuner.promoted")),L()}catch(t){if(Ot(t,{base:"/api/tables",wapi:w,proposalId:a,onView:()=>{G(a)},onRebased:()=>{b(o("tool.conflict.rebased")),L()}}))return;b(o("tool.common.promoteFail",{msg:t.message}),!0)}}async function ot(a){try{await C(w("/api/tables/archive"),{proposalId:a}),b(o("tool.tuner.archived")),L()}catch(t){b(o("tool.common.archiveFail",{msg:t.message}),!0)}}async function q(){let a;try{a=H()}catch(n){b(o("tool.tuner.readFail",{msg:n.message}),!0);return}const t=s("savelabel").value.trim()||o("tool.tuner.defaultLabel"),e={formatVersion:kt,id:"tables",name:t,tables:a,createdAt:Y(),updatedAt:Y()};try{await C(w("/api/tables/proposal"),{tables:e,label:t,createdBy:"user"}),b(o("tool.tuner.saved",{name:t})),L()}catch(n){b(o("tool.common.saveFail",{msg:n.message}),!0)}}function rt(a,t){const e=a.split("."),n=Number(t);if(e[0]==="tuning")p.tuning[e[1]]=n;else if(e[0]==="gen")p.genParams[e[1]]=n;else if(e[0]==="gim"){const i=p.gimmicks[e[1]];i&&(i[e[2]]=n),$=e[1],B()}else if(e[0]==="pack"){const i=p.enemyPacks[Number(e[1])];if(!i)return;if(e[2]==="name")i.name=t;else if(e[2]==="units"){const m=Lt(t);if(!m){b(o("tool.tuner.unitsBad"),!0);return}i.units=m}else i[e[2]]=n}else if(e[0]==="mat"){const i=p.materials[Number(e[1])];if(!i)return;e[2]==="id"||e[2]==="name"?i[e[2]]=t:i[e[2]]=n}else if(e[0]==="theme"){const i=p.themes[e[1]];if(!i)return;e[2]==="col"||e[2]==="mat"?i[e[2]]=t:e[2]==="water"?i.water=t:e[2]==="hazard"&&(i.hazard=t||null)}_()}s("form").addEventListener("input",a=>{const t=a.target,e=t.getAttribute("data-f");e&&rt(e,t.value)}),s("form").addEventListener("click",a=>{const t=a.target.closest("[data-act],[data-gimsel]");if(!t)return;const e=t.getAttribute("data-gimsel");if(e){$=e,v();return}const n=t.getAttribute("data-act"),i=Number(t.getAttribute("data-i")??-1);if(n==="pack-del")p.enemyPacks.splice(i,1);else if(n==="pack-add")p.enemyPacks.push({name:o("tool.tuner.newPack"),min:1,max:99,weight:2,units:[{role:"bruiser",n:1}]});else if(n==="mat-del")p.materials.splice(i,1);else if(n==="mat-add")p.materials.push({id:`mat${p.materials.length}`,name:o("tool.tuner.newMat"),min:1,value:10});else return;v(),_()});function z(){s("tab-curve").classList.toggle("on",x==="curve"),s("tab-table").classList.toggle("on",x==="table"),s("tab-feel").classList.toggle("on",x==="feel")}s("tab-curve").addEventListener("click",()=>{x="curve",z(),v(),j()}),s("tab-table").addEventListener("click",()=>{x="table",z(),v(),j()}),s("tab-feel").addEventListener("click",()=>{x="feel",z(),v(),j()}),s("reset").addEventListener("click",()=>{p=V(),v(),_(),b(o("tool.tuner.resetDone"))}),s("depth").addEventListener("input",()=>{u=Number(s("depth").value),s("depthv").textContent=`${u}F`,x==="curve"&&v(),_()}),s("seedbase").addEventListener("input",_),s("count").addEventListener("change",()=>{j(),M()}),s("regen").addEventListener("click",()=>{M()}),s("save").addEventListener("click",()=>{q()}),s("play").addEventListener("click",()=>{const a=Object.keys(O).length?`dungeon.html?pack=${encodeURIComponent(_t(O))}`:"dungeon.html";window.open(a,"_blank")}),s("grid").addEventListener("click",a=>{const t=a.target.closest(".thumb");t&&t.classList.toggle("zoom")});const st=T.events.on("tables",()=>{L()});window.cdTuner={draft:()=>p,patch:a=>{a(p),v(),_()},overrides:()=>H(),depth:a=>(u=a,s("depth").value=String(a),s("depthv").textContent=`${a}F`,M()),regen:()=>M(),perf:()=>F,save:()=>q(),refresh:()=>L()};const it=Ct(()=>{z(),v(),I(),W(O),L()}),lt=Q();return z(),v(),I(),lt&&b(o("tool.common.restored")),M(),L(),{applyParams(a){const t=Number(a.get("depth"));!Number.isFinite(t)||t<1||(u=Math.max(1,Math.min(30,Math.round(t))),s("depth").value=String(u),s("depthv").textContent=`${u}F`,x==="curve"&&v(),_())},dispose(){E++,clearTimeout(N),st(),it(),j(),J(null),delete window.cdTuner}}}};export{Kt as tunerPanel};
