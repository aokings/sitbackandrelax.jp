import{_ as U,r as dt,g as P,d as pt,a as ct,b as ut,c as mt,e as ft,f as ht,h as bt,i as vt,j as gt,k as xt,l as yt}from"./dungeon-DGPv831v.js";import{p as wt}from"./monster-x5Yq99HI.js";import{C as F}from"./tables-CNdyXHxR.js";import{T as $t}from"./tables-doc-H1DjZKok.js";import{f as J,p as kt,a as _t,g as V,b as R,u as Lt,E as Tt,s as St,o as jt,t as Mt,d as K}from"./tuner-core-Ckv1h2zf.js";import{r as Pt}from"./tool-shelf-CLTXCBc4.js";import{o as zt}from"./studio-BeknSLRD.js";import{c as Ot,t as o}from"./world-ui-B_HPJotp.js";import"./dungeon-types-ah7saRkX.js";import"./gamepack-Bq1NqM8d.js";import"./voxmodel-C0GaF4bJ.js";import"./scene-45PRHrjY.js";import"./three.module-DegpxYfP.js";import"./modulepreload-polyfill-B5Qt9EMX.js";import"./creator-auth-C2cecs39.js";const Ht=`<!-- ============================================================
...
`,c=g=>g.replace(/[&<>"]/g,T=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[T]),Z=()=>new Date().toISOString().replace(/\.\d+Z$/,"Z");try{const g=Object.assign({"../../tenants/masaru/comduct/prefabs/deep-throne.json":xt,"../../tenants/masaru/comduct/prefabs/interior-craft.json":gt,"../../tenants/masaru/comduct/prefabs/interior-equip.json":vt,"../../tenants/masaru/comduct/prefabs/interior-heal.json":bt,"../../tenants/masaru/comduct/prefabs/interior-recruit.json":ht,"../../tenants/masaru/comduct/prefabs/interior-shop.json":ft,"../../tenants/masaru/comduct/prefabs/manifest.json":mt,"../../tenants/masaru/comduct/prefabs/root-den.json":ut,"../../tenants/masaru/comduct/prefabs/shrine-guardian.json":ct}),T=[];for(const w of Object.keys(g).sort())w.endsWith("manifest.json")||T.push(wt(g[w].default??g[w]));yt(T)}catch(g){console.warn("truth プレハブの読込に失敗（プレハブ無しの束見で継続）",g)}const Ct={tuning:F.tuning,gen:F.genParams,gim:F.gimmicks},Zt={id:"tuner",async mount(g,T){const{wapi:w,postJson:H}=T.api;g.innerHTML=Ht,Ot(g);const s=a=>g.querySelector(`#${a}`);function b(a,t=!1){const e=s("msg");e.textContent=a,e.style.color=t?"#ff7a6e":"#7fd17f"}let p=J(),x="curve",$="trap",u=3,O={};function C(){return jt(p)}const D="cdTunerW2";function j(){try{sessionStorage.setItem(D,JSON.stringify({v:1,draft:p,tab:x,selGim:$,depth:u,seed:s("seedbase").value,count:s("count").value,label:s("savelabel").value}))}catch{}}function Y(){var a,t;try{const e=sessionStorage.getItem(D);if(!e)return!1;const n=JSON.parse(e);return n.v!==1||!((a=n.draft)!=null&&a.genParams)||!((t=n.draft)!=null&&t.gimmicks)?!1:(p=n.draft,x=n.tab??"curve",$=n.draft.gimmicks[n.selGim]?n.selGim:"trap",u=Math.max(1,Math.min(30,n.depth|0||3)),s("depth").value=String(u),s("depthv").textContent=`${u}F`,n.seed&&(s("seedbase").value=n.seed),n.count&&(s("count").value=n.count),n.label&&(s("savelabel").value=n.label),!0)}catch{return!1}}const f=(a,t,e,n,i=1,m="num")=>{const r=a.split(".").reduce((d,k)=>d==null?void 0:d[k],Ct);return`<input class="${m}${typeof r=="number"&&r!==t?" chg":""}" data-f="${a}" type="number" value="${t}" min="${e}" max="${n}" step="${i}" />`};function v(){const a=s("form");if(x==="curve"){const n=p.genParams,i=V(n,u),m=Object.keys(p.gimmicks).map(r=>{const l=p.gimmicks[r];return`<tr data-gim="${r}" class="${r===$?"sel":""}">
          ...
          <td><input class="wide" data-f="pack.${l}.units" type="text" value="${c(Lt(r.units))}" /></td>
          ...
          <div class="tblwrap"><table class="grid"><thead><tr><th>${c(o("tool.tuner.colTheme"))}</th><th>col</th><th>mat</th><th>water</th><th>hazard</th></tr></thead><tbody>${m}</tbody></table></div>`;return}const t=p.tuning,e=(n,i)=>`<h2>${c(n)}</h2>`+i.map(([m,r,l,d])=>`<div class="kv"><span class="lbl" style="min-width:9em">${m}</span>${f(`tuning.${m}`,t[m],r,l,d)}</div>`).join("");a.innerHTML=e(o("tool.tuner.feelPos"),[["posBack",1,3,.1],["posSide",1,3,.1],["posHigh",1,3,.1]])+e(o("tool.tuner.feelWave"),[["waveFirstAfter",0,30,1],["waveGapBase",1,30,1],["waveGapJitter",0,10,1],["waveCountCap",1,12,1],["monsterCap",4,40,1]])+e(o("tool.tuner.feelGold"),[["assaultGoldBase",0,99,1],["assaultGoldPerDepth",0,30,1]])+e(o("tool.tuner.feelMat"),[["matDropBase",0,100,1],["matDropPerDepth",0,20,1],["themeMatN",0,10,1]])+e(o("tool.tuner.feelBreath"),[["breathLen",1,9,1],["breathCd",1,20,1]])+e(o("tool.tuner.feelLarge"),[["largePerFloor",0,4,1],["largePerFloorDeep",0,6,1],["largeDeepFrom",1,99,1]])}function B(){const a=g.querySelector("#curvechart");if(!a)return;const t=a.getContext("2d"),e=a.width,n=a.height,i=30;t.clearRect(0,0,e,n),t.strokeStyle="#2b3547",t.lineWidth=1;for(const d of[.25,.5,.75])t.beginPath(),t.moveTo(0,n*d),t.lineTo(e,n*d),t.stroke();const m=d=>(d-1)/(i-1)*(e-8)+4,r=d=>n-6-d*(n-14),l=(d,k,y)=>{t.strokeStyle=k,t.lineWidth=y,t.beginPath();for(let L=1;L<=i;L++){const q=r(R(p.gimmicks[d],L));L===1?t.moveTo(m(L),q):t.lineTo(m(L),q)}t.stroke()};for(const d of Object.keys(p.gimmicks))d!==$&&l(d,"rgba(139,150,168,0.28)",1);p.gimmicks[$]&&l($,"#ffb347",2),t.strokeStyle="#ffd166",t.lineWidth=1,t.setLineDash([3,3]),t.beginPath(),t.moveTo(m(u),0),t.lineTo(m(u),n),t.stroke(),t.setLineDash([]),t.fillStyle="#ffb347",t.font="10px ui-monospace",t.fillText(`${$} @${u}F = ${Math.round(R(p.gimmicks[$],u)*100)}%`,6,11)}const h=3;let E=0,N="";function Q(a){let t=0;for(;P().floor.depth<a&&t++<a+30;){const e=P().actors.find(n=>n.kind==="hero");if(!e)return;e.pos={...P().floor.stairs},pt()}}function X(a){const t=P(),e=t.floor;a.width=e.w*h,a.height=e.h*h;const n=a.getContext("2d");let i=0;for(let r=0;r<e.h;r++)for(let l=0;l<e.w;l++){const d=e.tiles[r][l];d.chest&&!d.chest.opened&&i++,n.fillStyle=Mt(d),n.fillRect(l*h,r*h,h,h)}n.fillStyle="#ffffff",n.fillRect(e.stairs.x*h,e.stairs.y*h,h,h),e.upstairs&&(n.fillStyle="#9aa7bd",n.fillRect(e.upstairs.x*h,e.upstairs.y*h,h,h));let m=0;for(const r of t.actors)r.alive&&(r.kind==="monster"?(m++,n.fillStyle="#ff5a4a"):r.kind==="hero"?n.fillStyle="#ffe9a8":n.fillStyle="#6ab0ff",n.fillRect(r.pos.x*h,r.pos.y*h,h,h));return{mon:m,chest:i}}async function M(){const a=++E;let t;try{t=C()}catch(d){s("lint").innerHTML=`<div class="err">✗ ${c(d.message.slice(0,800))}</div>`;return}O=t;const e=Object.keys(t).length;s("lint").innerHTML=`<div class="ok">✓ ${c(o("tool.tuner.lintOk",{n:e}))}</div>`,W(t),U(e?t:null);const n=St(Number(s("seedbase").value)||0,Number(s("count").value)),i=s("grid");i.innerHTML=n.map((d,k)=>`<div class="thumb" data-i="${k}"><canvas></canvas><div class="cap">#${d} …</div></div>`).join("");const m=performance.now();for(let d=0;d<n.length;d++){if(a!==E)return;dt(n[d]),Q(u);const k=i.children[d],y=X(k.querySelector("canvas"));k.querySelector(".cap").textContent=`#${n[d]}  ${o("tool.tuner.capMon")}${y.mon} ${o("tool.tuner.capChest")}${y.chest}${P().floor.monsterHouse?" MH!":""}${P().floor.sealRoom?" 封":""}`,d%4===3&&await new Promise(L=>setTimeout(L))}const r=Math.round(performance.now()-m);N=o("tool.tuner.perf",{n:n.length,d:u,ms:r}),s("perf").textContent=N;const l=V(p.genParams,u);s("gensum").textContent=o("tool.tuner.atDepth",{d:u,rooms:l.maxRooms,loops:l.loops})}let A=0;function _(){clearTimeout(A),A=window.setTimeout(()=>{M()},180),j()}function W(a){const t=Object.keys(a);s("ovsum").textContent=t.length?o("tool.tuner.ovSome",{list:t.join(", ")}):o("tool.tuner.ovNone"),s("ovjson").textContent=t.length?JSON.stringify(a,null,1):"{}"}function G(){const a=[["#39455e",o("tool.tuner.lgWall")],["#141b2a",o("tool.tuner.lgFloor")],["#2a5f9e",o("tool.tuner.lgWater")],["#e8c15a",o("tool.tuner.lgChest")],["#c05a8a",o("tool.tuner.lgTrap")],["#a87b2a",o("tool.tuner.lgLocked")],["#6a4a8a",o("tool.tuner.lgSecret")],["#ff5a4a",o("tool.tuner.lgMon")],["#ffe9a8",o("tool.tuner.lgHero")],["#ffffff",o("tool.tuner.lgStairs")]];s("legend").innerHTML=a.map(([t,e])=>`<span><i style="background:${t}"></i>${c(e)}</span>`).join("")}async function S(){try{const[a,t]=await Promise.all([fetch(w("/api/tables/truth")).then(e=>e.json()),fetch(w("/api/tables/proposals")).then(e=>e.json())]);tt(a.docs??[]),et(t.proposals??[])}catch{s("truth").innerHTML=`<div class="live">${c(o("tool.tuner.noApi"))}</div>`,s("proposals").innerHTML=""}}function tt(a){var i;const t=s("truth");if(!a.length){t.innerHTML=`<div class="live">${c(o("tool.tuner.truthEmpty"))}</div>`;return}const e=a[0],n=Object.keys(e.tables).length;t.innerHTML=`<div class="card"><b>${c(e.name)}</b><span class="id">${c(e.updatedAt)}・${n} ${c(o("tool.tuner.ovFields"))}</span>
        <div class="row"><button type="button" data-act="truth-load">${c(o("tool.tuner.load"))}</button></div></div>`,(i=t.querySelector('[data-act="truth-load"]'))==null||i.addEventListener("click",()=>{p=K(e.tables),v(),_(),b(o("tool.tuner.loaded",{name:e.name}))})}function et(a){Pt(s("proposals"),a,{tag:()=>o("tool.tuner.capTag"),onView:t=>{nt(t)},onPromote:t=>{at(t)},onArchive:t=>{ot(t)}})}async function nt(a){var t;try{const n=(t=(await(await fetch(w(`/api/tables/proposal?id=${encodeURIComponent(a)}`))).json()).docs)==null?void 0:t[0];if(!n)throw new Error("proposal is empty");p=K(n.tables),v(),_(),b(o("tool.tuner.loaded",{name:n.name}))}catch(e){b(e.message,!0)}}async function at(a){try{await H(w("/api/tables/promote"),{proposalId:a}),b(o("tool.tuner.promoted")),S()}catch(t){b(t.message,!0)}}async function ot(a){try{await H(w("/api/tables/archive"),{proposalId:a}),b(o("tool.tuner.archived")),S()}catch(t){b(t.message,!0)}}async function I(){let a;try{a=C()}catch(n){b(n.message,!0);return}const t=s("savelabel").value.trim()||o("tool.tuner.defaultLabel"),e={formatVersion:$t,id:"tables",name:t,tables:a,createdAt:Z(),updatedAt:Z()};try{await H(w("/api/tables/proposal"),{tables:e,label:t,createdBy:"user"}),b(o("tool.tuner.saved",{name:t})),S()}catch(n){b(n.message,!0)}}function rt(a,t){const e=a.split("."),n=Number(t);if(e[0]==="tuning")p.tuning[e[1]]=n;else if(e[0]==="gen")p.genParams[e[1]]=n;else if(e[0]==="gim"){const i=p.gimmicks[e[1]];i&&(i[e[2]]=n),$=e[1],B()}else if(e[0]==="pack"){const i=p.enemyPacks[Number(e[1])];if(!i)return;if(e[2]==="name")i.name=t;else if(e[2]==="units"){const m=_t(t);if(!m){b(o("tool.tuner.unitsBad"),!0);return}i.units=m}else i[e[2]]=n}else if(e[0]==="mat"){const i=p.materials[Number(e[1])];if(!i)return;e[2]==="id"||e[2]==="name"?i[e[2]]=t:i[e[2]]=n}else if(e[0]==="theme"){const i=p.themes[e[1]];if(!i)return;e[2]==="col"||e[2]==="mat"?i[e[2]]=t:e[2]==="water"?i.water=t:e[2]==="hazard"&&(i.hazard=t||null)}_()}s("form").addEventListener("input",a=>{const t=a.target,e=t.getAttribute("data-f");e&&rt(e,t.value)}),s("form").addEventListener("click",a=>{const t=a.target.closest("[data-act],[data-gimsel]");if(!t)return;const e=t.getAttribute("data-gimsel");if(e){$=e,v();return}const n=t.getAttribute("data-act"),i=Number(t.getAttribute("data-i")??-1);if(n==="pack-del")p.enemyPacks.splice(i,1);else if(n==="pack-add")p.enemyPacks.push({name:o("tool.tuner.newPack"),min:1,max:99,weight:2,units:[{role:"bruiser",n:1}]});else if(n==="mat-del")p.materials.splice(i,1);else if(n==="mat-add")p.materials.push({id:`mat${p.materials.length}`,name:o("tool.tuner.newMat"),min:1,value:10});else return;v(),_()});function z(){s("tab-curve").classList.toggle("on",x==="curve"),s("tab-table").classList.toggle("on",x==="table"),s("tab-feel").classList.toggle("on",x==="feel")}s("tab-curve").addEventListener("click",()=>{x="curve",z(),v(),j()}),s("tab-table").addEventListener("click",()=>{x="table",z(),v(),j()}),s("tab-feel").addEventListener("click",()=>{x="feel",z(),v(),j()}),s("reset").addEventListener("click",()=>{p=J(),v(),_(),b(o("tool.tuner.resetDone"))}),s("depth").addEventListener("input",()=>{u=Number(s("depth").value),s("depthv").textContent=`${u}F`,x==="curve"&&v(),_()}),s("seedbase").addEventListener("input",_),s("count").addEventListener("change",()=>{j(),M()}),s("regen").addEventListener("click",()=>{M()}),s("save").addEventListener("click",()=>{I()}),s("play").addEventListener("click",()=>{const a=Object.keys(O).length?`dungeon.html?pack=${encodeURIComponent(kt(O))}`:"dungeon.html";window.open(a,"_blank")}),s("grid").addEventListener("click",a=>{const t=a.target.closest(".thumb");t&&t.classList.toggle("zoom")});const st=T.events.on("tables",()=>{S()});window.cdTuner={draft:()=>p,patch:a=>{a(p),v(),_()},overrides:()=>C(),depth:a=>(u=a,s("depth").value=String(a),s("depthv").textContent=`${a}F`,M()),regen:()=>M(),perf:()=>N,save:()=>I(),refresh:()=>S()};const it=zt(()=>{z(),v(),G(),W(O),S()}),lt=Y();return z(),v(),G(),lt&&b(o("tool.common.restored")),M(),S(),{applyParams(a){const t=Number(a.get("depth"));!Number.isFinite(t)||t<1||(u=Math.max(1,Math.min(30,Math.round(t))),s("depth").value=String(u),s("depthv").textContent=`${u}F`,x==="curve"&&v(),_())},dispose(){E++,clearTimeout(A),st(),it(),j(),U(null),delete window.cdTuner}}}};export{Zt as tunerPanel};
