import{p as I}from"./creator-auth-C2cecs39.js";import{c as La,a as _,t as s,D as sa,w as L,C as $a,m as Sa}from"./world-ui-C6E9eyQU.js";import{b as A,d as M,S as la,j as ra,c as da,o as Ea,l as Ca,e as Na,f as ia,g as pa,h as ca,a as Z,i as Ta,k as Q,s as W,L as aa,m as wa,P as za}from"./studio-CzlVzAfP.js";import{r as Fa}from"./tool-shelf-BHxHiMoE.js";import{T as Ha}from"./tables-doc-Dd9M6e7G.js";import"./tables-taZ1PZre.js";const Ia=`<!-- ============================================================
...
`,c=C=>C.replace(/[&<>"]/g,h=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[h]),ma=()=>new Date().toISOString().replace(/\.\d+Z$/,"Z"),Aa=[["worlds.laws.g.pos",["posBack","posSide","posHigh"]],["worlds.laws.g.wave",["waveFirstAfter","waveGapBase","waveGapJitter","waveCountCap"]],["worlds.laws.g.cap",["monsterCap"]],["worlds.laws.g.breath",["breathLen","breathCd"]]],_a={id:"worlds",async mount(C,h){C.innerHTML=Ia,La(C);const r=a=>C.querySelector(`#${a}`);function v(a,e=!1){const o=r("say");o.textContent=a,o.className=e?"err":""}let U="masaru",$=[],y=!1,x="fork",G=sa;function P(){for(const a of Array.from(C.querySelectorAll(".tplcard")))a.classList.toggle("on",a.dataset.tpl===x);r("fldFromLbl").hidden=x!=="from"}r("tplRow").addEventListener("click",a=>{const e=a.target.closest(".tplcard");e!=null&&e.dataset.tpl&&(x=e.dataset.tpl,P())});function q(a,e){if(e===h.world()){h.navigate(a);return}const o=h.standalone?new URL("studio.html",window.location.href):new URL(window.location.href);o.search="",o.searchParams.set("panel",a),o.searchParams.set("world",e),window.location.href=o.toString()}function B(){const a=r("worlds");if(a.innerHTML="",!$.length){const l=document.createElement("div");if(l.className="live",l.textContent=s("worlds.none"),a.appendChild(l),!y&&_()!==""){const n=document.createElement("a");n.className="btn on",n.target="_blank",n.rel="noopener",n.href=Sa(_(),"worlds"),n.textContent=`🐙 ${s("worlds.hostedCta")}`,a.appendChild(n)}if(y){const n=document.createElement("button");n.type="button",n.className="on",n.textContent=s("worlds.emptyCta"),n.addEventListener("click",()=>{r("fldId").focus();const d=r("wapp");d.scrollTo({top:d.scrollHeight,behavior:"smooth"})}),a.appendChild(n)}return}for(const l of $){const n=document.createElement("div");n.className="card";const d=document.createElement("div"),N=document.createElement("b");N.textContent=l.title,d.appendChild(N);const E=document.createElement("div");E.className="key",E.textContent=l.key;const O=document.createElement("div");O.className="tagrow";const V=document.createElement("span");V.className="tag"+(l.publishedAt?" pub":""),V.textContent=l.publishedAt?s("worlds.statusPublished"):s("worlds.statusDraft"),O.appendChild(V);const na=l.forkedFrom;if(na){const w=document.createElement("span");w.className="tag",w.textContent=s("worlds.forkedFrom",{world:na}),O.appendChild(w)}n.append(d,E,O);const K=document.createElement("div");K.className="row";for(const w of za){if(w.id==="worlds")continue;const H=document.createElement("button");H.type="button",H.className="btn",H.textContent=`${w.icon} ${s(w.titleKey)}`,H.addEventListener("click",()=>q(w.id,l.key)),K.appendChild(H)}n.appendChild(K);const X=document.createElement("div");X.className="row";const z=document.createElement("button");z.type="button",z.className="on",z.textContent=s("worlds.publish"),z.addEventListener("click",()=>{(async()=>{try{const w=await I("/api/worlds/publish",{world:l.key});v(s("worlds.published",{path:w.packPath})),await j()}catch(w){v(s("worlds.publishFail",{msg:w.message}),!0)}})()});const D=document.createElement("button");D.type="button",D.textContent=s("tool.common.dup"),D.addEventListener("click",()=>{x="from",P(),r("fldFrom").value=l.key,r("fldId").focus();const w=r("wapp");w.scrollTo({top:w.scrollHeight,behavior:"smooth"})});const F=document.createElement("a");F.className="btn",F.textContent=s("worlds.play"),F.href=`dungeon.html?pack=${encodeURIComponent(`packs/${l.packName}.json`)}`,F.target="_blank",X.append(z,D,F),n.appendChild(X),a.appendChild(n)}const e=r("fldFrom"),o=e.value;e.innerHTML="";const t=document.createElement("option");t.value="",t.textContent=s("worlds.fromEmpty"),e.appendChild(t);for(const l of $){const n=document.createElement("option");n.value=l.key,n.textContent=`${l.title}（${l.key}）`,e.appendChild(n)}[...e.options].some(l=>l.value===o)&&(e.value=o)}function J(){r("acct").textContent=y?U:s("worlds.acctLocal"),r("acctNote").textContent=y?s("worlds.acctNote",{n:$.length}):""}async function j(){const a=await $a();if(!a){_()!==""?v(s("worlds.hostedNeed")):v(s("worlds.loadFail",{msg:"API unreachable"}),!0),y=!1,$=[],J(),B();return}y=!0,U=a.accountId,$=a.worlds,G=a.defaultWorldKey??sa,J(),B()}r("createForm").addEventListener("submit",a=>{if(a.preventDefault(),!y&&_()!==""){v(s("worlds.hostedNeed"),!0);return}const e=r("fldId").value.trim(),o=r("fldTitle").value.trim()||e,t=x==="fork"?G:x==="from"?r("fldFrom").value:void 0;if(x==="from"&&!t){v(s("worlds.tpl.fromNeed"),!0);return}(async()=>{try{const l=await I("/api/worlds",{id:e,title:o,...t?{from:t}:{}});v(s("worlds.created",{key:l.key}));const n=document.createElement("button");n.type="button",n.className="on",n.style.marginLeft="8px",n.textContent=s("worlds.openNew",{key:l.key}),n.addEventListener("click",()=>q("hub",l.key)),r("say").appendChild(n),r("fldId").value="",r("fldTitle").value="",await j()}catch(l){v(s("worlds.createFail",{msg:l.message}),!0)}})()});let u="home",p=A(),i=M(),b=!1,f=null;function m(a,e=!1){const o=r("lawsSay");o.textContent=a,o.className=e?"err":"",o.style.color=e?"#e08a8a":"#8fd6a3"}function T(){r("wtab-home").classList.toggle("on",u==="home"),r("wtab-laws").classList.toggle("on",u==="laws"),r("whome").hidden=u!=="home",r("wlaws").hidden=u!=="laws"}const ea="cdWorldsLawsM6";function g(){try{sessionStorage.setItem(ea,JSON.stringify({v:2,tab:u,laws:p,sys:i,dirty:b,label:r("lawsLabel").value}))}catch{}}function ua(){var a,e;try{const o=sessionStorage.getItem(ea);if(!o)return!1;const t=JSON.parse(o);if(t.v!==1&&t.v!==2||typeof((a=t.laws)==null?void 0:a.monsterCap)!="number")return!1;u=t.tab==="laws"?"laws":"home";const l=A();for(const d of aa)typeof t.laws[d]=="number"&&(l[d]=t.laws[d]);p=l;const n=M();for(const[,d]of wa)for(const N of d){const E=(e=t.sys)==null?void 0:e[N];E!=null&&typeof E==typeof n[N]&&(n[N]=E)}return i=n,b=!!t.dirty,t.label&&(r("lawsLabel").value=t.label),!0}catch{return!1}}function k(){const a=A();r("lawsForm").innerHTML=Aa.map(([e,o])=>`<div class="lawgrp"><h3>${c(s(e))}</h3>${o.map(t=>{const l=Ta[t];return`<div class="lawrow"><code>${t}</code><span class="lbl">${c(s(`worlds.laws.f.${t}`))}</span><input data-law="${t}" type="number" value="${p[t]}" min="${l.min}" max="${l.max}" step="${l.step}" class="${p[t]!==a[t]?"chg":""}" /><span class="rng">${l.min}–${l.max}</span></div>`}).join("")}</div>`).join(""),fa(),R(),Y()}function ba(a){const e=la[a],o=i[a]!==M()[a],t=`<code>${a}</code><span class="lbl">${c(s(`worlds.laws.sys.f.${a}`))}</span>`;if(e.kind==="toggle")return`<div class="lawrow${o?" chg":""}">${t}<input data-sys="${a}" type="checkbox"${i[a]?" checked":""} /></div>`;if(e.kind==="enum"){const l=(e.options??[]).map(n=>`<option value="${n}"${i[a]===n?" selected":""}>${c(s(`worlds.laws.sys.opt.${a}.${n}`))}</option>`).join("");return`<div class="lawrow${o?" chg":""}">${t}<select data-sys="${a}" class="${o?"chg":""}">${l}</select></div>`}return`<div class="lawrow">${t}<input data-sys="${a}" type="number" value="${i[a]}" min="${e.min}" max="${e.max}" step="${e.step}" class="${o?"chg":""}" /><span class="rng">${e.min}–${e.max}</span></div>`}function fa(){r("sysForm").innerHTML=wa.map(([a,e])=>`<div class="lawgrp"><h3>${c(s(`worlds.laws.sys.g.${a}`))}</h3>${e.map(ba).join("")}</div>`).join("")}function R(){const a=r("lawsLint"),e=r("lawsSave"),o=[...Ca(p),...Na(i)];if(o.length)return a.innerHTML=`<div class="err">${o.map(t=>c(s("worlds.laws.rangeErr",{field:t.field,min:t.min,max:t.max}))).join(`
`)}</div>`,e.disabled=!0,!1;try{ia(pa({},p),i)}catch(t){return a.innerHTML=`<div class="err">✗ ${c(t.message.slice(0,600))}</div>`,e.disabled=!0,!1}return a.innerHTML=`<div class="ok">✓ ${c(s("worlds.laws.lintOk",{n:ca(p).length+Z(i).length}))}</div>`,e.disabled=!1,!0}function Y(){const a=da(p),e=ra(i),o=[...a.notes,...e.notes],t=o.some(n=>n.level==="warn")?"warn":o.length?"note":"ok",l=s(t==="ok"?"worlds.laws.meterOk":t==="note"?"worlds.laws.meterNote":"worlds.laws.meterWarn");r("lawsMeter").innerHTML=`<div class="meter ${t}"><b class="lv">${c(l)}</b>${o.length?`<ul>${o.map(n=>`<li>${c(s(`worlds.laws.n.${n.key}`))}</li>`).join("")}</ul>`:""}</div>`}function ta(a){var l;const e=r("lawsTruth");if(!a){e.innerHTML=`<div class="live">${c(s("worlds.laws.truthEmpty"))}</div>`;return}const o=aa.filter(n=>{var d;return typeof((d=a.tables.tuning)==null?void 0:d[n])=="number"}).length+(a.tables.systems?Z(W(a.tables)).length:0),t=[...Object.keys(a.tables).filter(n=>n!=="tuning"&&n!=="systems"),...Object.keys(a.tables.tuning??{}).filter(n=>!aa.includes(n)).map(n=>`tuning.${n}`)];e.innerHTML=`<div class="card"><b>${c(a.name)}</b><span class="id">${c(a.updatedAt)}・${o} ${c(s("worlds.laws.ovFields"))}</span><span class="id">${c(t.length?s("worlds.laws.keepNote",{list:t.join(", ")}):s("worlds.laws.keepNone"))}</span><div class="row"><button type="button" data-act="laws-truth-load">${c(s("worlds.laws.load"))}</button></div></div>`,(l=e.querySelector('[data-act="laws-truth-load"]'))==null||l.addEventListener("click",()=>{p=Q(a.tables),i=W(a.tables),b=!1,k(),m(s("worlds.laws.loaded",{name:a.name})),g()})}async function S(){var a;r("lawsWorld").textContent=h.world();try{const[e,o]=await Promise.all([fetch(L("/api/tables/truth")).then(t=>t.json()),fetch(L("/api/tables/proposals")).then(t=>t.json())]);f=((a=e.docs)==null?void 0:a[0])??null,b||(p=Q((f==null?void 0:f.tables)??{}),i=W((f==null?void 0:f.tables)??{}),k()),ta(f),Fa(r("lawsProposals"),o.proposals??[],{tag:()=>s("worlds.laws.capTag"),onView:t=>{ga(t)},onPromote:t=>{xa(t)},onArchive:t=>{ha(t)}})}catch{f=null,r("lawsTruth").innerHTML=`<div class="live">${c(s("worlds.laws.noApi"))}</div>`,r("lawsProposals").innerHTML=""}}async function ga(a){var e;try{const t=(e=(await(await fetch(L(`/api/tables/proposal?id=${encodeURIComponent(a)}`))).json()).docs)==null?void 0:e[0];if(!t)throw new Error("proposal is empty");p=Q(t.tables),i=W(t.tables),b=!0,k(),m(s("worlds.laws.loaded",{name:t.name})),g()}catch(o){m(o.message,!0)}}async function xa(a){try{await I(L("/api/tables/promote"),{proposalId:a}),m(s("tool.common.adoptedNote")),S()}catch(e){m(s("tool.common.promoteFail",{msg:e.message}),!0)}}async function ha(a){try{await I(L("/api/tables/archive"),{proposalId:a}),m(s("tool.common.archived")),S()}catch(e){m(s("tool.common.archiveFail",{msg:e.message}),!0)}}async function oa(){var n;if(!R()){m(s("worlds.laws.saveNoValid"),!0);return}let a=f;try{a=((n=(await(await fetch(L("/api/tables/truth"))).json()).docs)==null?void 0:n[0])??null}catch{}let e;try{e=ia(pa((a==null?void 0:a.tables)??{},p),i)}catch(d){m(d.message,!0);return}const o=[...ca(p),...Z(i)],t=r("lawsLabel").value.trim()||s("worlds.laws.defaultLabel",{list:o.length?o.join(", "):"—"}),l={formatVersion:Ha,id:"tables",name:t,tables:e,createdAt:(a==null?void 0:a.createdAt)??ma(),updatedAt:ma()};try{await I(L("/api/tables/proposal"),{tables:l,label:t,createdBy:"user"}),m(s("worlds.laws.saved",{name:t})),S()}catch(d){m(s("tool.common.saveFail",{msg:d.message}),!0)}}r("wtab-home").addEventListener("click",()=>{u="home",T(),g()}),r("wtab-laws").addEventListener("click",()=>{u="laws",T(),g()}),r("lawsForm").addEventListener("input",a=>{const e=a.target,o=e.getAttribute("data-law");o&&(p[o]=Number(e.value),b=!0,e.classList.toggle("chg",p[o]!==A()[o]),R(),Y(),g())}),r("sysForm").addEventListener("input",a=>{var d;const e=a.target,o=e.getAttribute("data-sys");if(!o)return;const t=la[o],l=i;t.kind==="toggle"?l[o]=e.checked:t.kind==="enum"?l[o]=e.value:l[o]=Number(e.value),b=!0;const n=i[o]!==M()[o];t.kind!=="toggle"&&e.classList.toggle("chg",n),(d=e.closest(".lawrow"))==null||d.classList.toggle("chg",n),R(),Y(),g()}),r("lawsSave").addEventListener("click",()=>{oa()}),r("lawsReset").addEventListener("click",()=>{p=A(),i=M(),b=!0,k(),m(s("worlds.laws.resetDone")),g()});const va=h.events.on("tables",()=>{S()});window.cdWorlds={reload:()=>j(),worlds:()=>$,account:()=>U,apiOk:()=>y,api:a=>L(a),open:(a,e)=>q(a,e??h.world()),tpl:a=>(a&&(x=a,P()),x),flagship:()=>G,laws:{tab:a=>{u=a,T()},draft:()=>({...p}),set:(a,e)=>{p[a]=e,b=!0,k(),g()},judge:()=>da(p),sysDraft:()=>({...i}),setSys:(a,e)=>{i[a]=e,b=!0,k(),g()},judgeSys:()=>ra(i),save:()=>oa(),refresh:()=>S()}};const ya=Ea(()=>{J(),B(),k(),ta(f),S()}),ka=ua();return T(),P(),k(),ka&&b&&m(s("tool.common.restored")),j(),S(),{applyParams(a){const e=a.get("tab");e==="laws"?u="laws":e==="home"&&(u="home"),T()},dispose(){va(),ya(),g(),delete window.cdWorlds}}}};export{_a as worldsPanel};
