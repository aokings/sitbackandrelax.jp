import{W as Ke,A as Tn,S as Ze,P as qe,H as Pn,D as ye,a as Nn,L as In,B as Qe,l as qt,N as tn,M as Dt,Q as en,t as $n,V as St,T as Bt,U as Qt,E as nn,z as On,C as on,s as we,X as Fn,u as an,Y as Dn,e as Hn,G as Bn,m as Rn,Z as Vn,R as An,J as jn}from"./render-voxel-PcSYZtX2.js";import{e as Le,V as Un,p as mt,m as Rt,f as Gn,g as Wn,a as Yn}from"./voxmodel-BixVuI7N.js";import{E as Xn}from"./forge-edit-CffY6cQY.js";import{a as _n}from"./touch-input-D1HYJrcI.js";import{i as Jn,b as Ht,r as Kn,a as Zn}from"./tool-shelf-CPrJ-OK1.js";import{M as qn}from"./dungeon-CzQMf0gi.js";import{b as Qn}from"./creator-auth-q666652w.js";import{o as to}from"./studio-C4l3Mpju.js";import{b as eo,t as u,c as ft}from"./i18n-D_93wjAr.js";import"./dungeon-types-JT-53Zyi.js";import"./schemas-BBGxizon.js";import"./preload-helper-Dp1pzeXC.js";import"./gamepack-Ch4DClsv.js";import"./tables-B68AojDj.js";function P(s){let l=0,d=0;for(const m of s.vox.layers){d=Math.max(d,m.length);for(const n of m)l=Math.max(l,n.length)}return{w:l,d,h:s.vox.layers.length}}function Ee(s){const{w:l,d}=P(s);s.vox.layers=s.vox.layers.map(m=>{const n=m.map(a=>a.padEnd(l,"."));for(;n.length<d;)n.push(".".repeat(l));return n})}function pt(s,l,d,m){var a,p;const n=(p=(a=s.vox.layers[d])==null?void 0:a[m])==null?void 0:p[l];return n&&n!=="."?n:null}function ln(s,l,d,m,n){var p;const a=(p=s.vox.layers[d])==null?void 0:p[m];return a==null||l<0||l>=a.length||a[l]===n?!1:(s.vox.layers[d][m]=a.slice(0,l)+n+a.slice(l+1),!0)}function Vt(s,l,d,m,n,a=!1){const p=n??".";let g=ln(s,l,d,m,p);if(a){const{w:h}=P(s),f=h-1-l;f!==l&&(g=ln(s,f,d,m,p)||g)}return g}function Me(s,l,d,m,n,a=!1){var I,L;const p=s.vox.layers[l];if(!p)return!1;const g=(I=p[m])==null?void 0:I[d];if(g==null||g===(n??"."))return!1;let f=!1;const k=[[d,m]],M=new Set;for(;k.length;){const[z,T]=k.pop(),ot=z+","+T;M.has(ot)||(M.add(ot),((L=s.vox.layers[l][T])==null?void 0:L[z])===g&&(f=Vt(s,z,l,T,n,a)||f,k.push([z+1,T],[z-1,T],[z,T+1],[z,T-1])))}return f}function sn(s,l,d,m){Ee(s);const n=P(s);let a=0;s.vox.layers.forEach((g,h)=>g.forEach((f,k)=>{for(let M=0;M<f.length;M++)f[M]!=="."&&(h>=m||k>=d||M>=l)&&a++}));const p=[];for(let g=0;g<m;g++){const h=[];for(let f=0;f<d;f++){const k=g<n.h&&f<n.d?s.vox.layers[g][f]:"";h.push(k.slice(0,l).padEnd(l,"."))}p.push(h)}return s.vox.layers=p,{dropped:a}}const no="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";function rn(s,l){const d=l.toLowerCase();for(const[m,n]of Object.entries(s.vox.pal))if(n.toLowerCase()===d)return m;for(const m of no)if(!(m in s.vox.pal))return s.vox.pal[m]=d,m;return null}function cn(s,l,d){if(!(l in s.vox.pal))return!1;const m=d.toLowerCase();if(s.vox.pal[l].toLowerCase()===m)return!1;const n=s.vox.pal[l].toLowerCase();return s.vox.pal[l]=m,s.glow=s.glow.map(a=>a.toLowerCase()===n?m:a),!0}function dn(s,l,d){const m=l.toLowerCase(),n=s.glow.some(a=>a.toLowerCase()===m);return d===n?!1:(d?s.glow.push(m):s.glow=s.glow.filter(a=>a.toLowerCase()!==m),!0)}function Se(s){const l=new Map;for(const d of s.vox.layers)for(const m of d)for(const n of m)n!=="."&&l.set(n,(l.get(n)??0)+1);return l}function ke(s){let l=0;for(const d of Se(s).values())l+=d;return l}const Ce=["x","y","z"];function te(s,l){return l==="x"?s.w:l==="y"?s.h:s.d}function pn(s,l,d){return{x:s.x/d+l.w/2,y:s.y/d,z:s.z/d+l.d/2}}function ze(s,l,d){return{x:(s.x+.5-l.w/2)*d,y:(s.y+.5)*d,z:(s.z+.5-l.d/2)*d}}function Te(s,l,d,m,n){const a=pn(s,d,m),p={x:l.x/m,y:l.y/m,z:l.z/m};let g=0,h=Number.POSITIVE_INFINITY,f=null;for(const b of Ce){const $=te(d,b),at=a[b],C=p[b];if(Math.abs(C)<1e-12){if(at<0||at>$)return null;continue}let H=-at/C,A=($-at)/C;const it=C>0?-1:1;if(H>A){const lt=H;H=A,A=lt}H>g&&(g=H,f={axis:b,sign:it}),A<h&&(h=A)}if(g>h||h<0)return null;const k=1e-7,M=g<=0,I=M?0:g+k,L={x:a.x+p.x*I,y:a.y+p.y*I,z:a.z+p.z*I},z={x:Math.min(d.w-1,Math.max(0,Math.floor(L.x))),y:Math.min(d.h-1,Math.max(0,Math.floor(L.y))),z:Math.min(d.d-1,Math.max(0,Math.floor(L.z)))};let T=M?null:f;const ot={x:Math.sign(p.x),y:Math.sign(p.y),z:Math.sign(p.z)},F={x:p.x!==0?Math.abs(1/p.x):Number.POSITIVE_INFINITY,y:p.y!==0?Math.abs(1/p.y):Number.POSITIVE_INFINITY,z:p.z!==0?Math.abs(1/p.z):Number.POSITIVE_INFINITY},S={x:0,y:0,z:0};for(const b of Ce)p[b]>0?S[b]=I+(z[b]+1-L[b])/p[b]:p[b]<0?S[b]=I+(z[b]-L[b])/p[b]:S[b]=Number.POSITIVE_INFINITY;const V=d.w+d.h+d.d+3;for(let b=0;b<=V;b++){if(n(z.x,z.y,z.z))return{cell:{...z},normal:T?{...T}:null};let $="x";if(S.y<S[$]&&($="y"),S.z<S[$]&&($="z"),S[$]>h+k||(z[$]+=ot[$],z[$]<0||z[$]>=te(d,$)))return null;T={axis:$,sign:ot[$]>0?-1:1},S[$]+=F[$]}return null}function ee(s,l,d,m,n){const a=pn(s,m,n),p={x:l.x/n,y:l.y/n,z:l.z/n},g=p[d.axis];if(Math.abs(g)<1e-9)return null;const h=(d.k+.5-a[d.axis])/g;if(h<=0)return null;const f={x:a.x+p.x*h,y:a.y+p.y*h,z:a.z+p.z*h},k={x:Math.floor(f.x),y:Math.floor(f.y),z:Math.floor(f.z)};k[d.axis]=d.k;for(const M of Ce)if(M!==d.axis&&(k[M]<0||k[M]>=te(m,M)))return null;return k}function oo(s,l,d,m,n,a,p,g){var M;const h={axis:"y",k:p};if(g){const I=ee(s,l,h,m,n);return I?{cell:I,plane:h,surface:!1}:null}const f=Te(s,l,m,n,a);if(f){if(d==="paint")return{cell:f.cell,plane:null,surface:!0};if(d==="erase"||d==="fill"){const L=((M=f.normal)==null?void 0:M.axis)??"y";return{cell:f.cell,plane:{axis:L,k:f.cell[L]},surface:!1}}if(!f.normal)return null;const I={...f.cell};return I[f.normal.axis]+=f.normal.sign,{cell:I,plane:{axis:f.normal.axis,k:I[f.normal.axis]},surface:!1}}const k=ee(s,l,h,m,n);return k?{cell:k,plane:h,surface:!1}:null}function ao(s,l){const d=Math.max(Math.abs(l.x-s.x),Math.abs(l.y-s.y),Math.abs(l.z-s.z)),m=[];for(let n=1;n<=d;n++)m.push({x:Math.round(s.x+(l.x-s.x)*(n/d)),y:Math.round(s.y+(l.y-s.y)*(n/d)),z:Math.round(s.z+(l.z-s.z)*(n/d))});return m}function io(s,l){const d=[];for(let m=Math.min(s.x,l.x);m<=Math.max(s.x,l.x);m++)for(let n=Math.min(s.y,l.y);n<=Math.max(s.y,l.y);n++)for(let a=Math.min(s.z,l.z);a<=Math.max(s.z,l.z);a++)d.push({x:m,y:n,z:a});return d}const lo=`<!-- ============================================================
...
`;function J(){return new Date().toISOString().replace(/\.\d+Z$/,"Z")}const nt=s=>s.replace(/[&<>"]/g,l=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[l]),B=new Map;{const s=l=>({kind:"monster",name:l.name,size:l.size});for(const l of qn){const d=Vn(s(l));B.has(d)||B.set(d,l.name)}for(const[l,d]of[["caster","術士"],["sentry","番兵"],["nest","魔物の巣"],["warden","番人"],["boss","封印のボス"]])B.has(l)||B.set(l,d)}function so(){try{return Rt("goblin",B.get("goblin")??"ゴブリン",Bt.goblin,Qt.goblin??[],J())}catch{return Le("goblin","ゴブリン",11,6,12,J())}}const ko={id:"sculpt",async mount(s,l){const{wapi:d,postJson:m}=l.api;s.innerHTML=lo,eo(s);const n=t=>s.querySelector(`#${t}`);let a=so(),p="a",g="brush",h=0,f=!1,k=!0,M="live",I=!1,L="carve",z=!1;const T=new Xn,ot="cdSculptP4";let F=.55,S=.35,V=2.2;function b(){try{sessionStorage.setItem(ot,JSON.stringify({v:1,model:a,brush:p,shape:g,layer:h,mirror:f,onion:k,lightMode:M,ttool:L,layerLock:z,cam:{yaw:F,pitch:S,dist:V}}))}catch{}}function $(){var t,e;try{const o=sessionStorage.getItem(ot);if(!o)return!1;const i=JSON.parse(o);return i.v!==1||!((e=(t=i.model)==null?void 0:t.vox)!=null&&e.layers)?!1:(a=i.model,p=i.brush??"a",g=i.shape??"brush",h=i.layer??0,f=!!i.mirror,k=i.onion!==!1,M=i.lightMode??"live",L=i.ttool??"carve",z=!!i.layerLock,i.cam&&(F=i.cam.yaw,S=i.cam.pitch,V=i.cam.dist),!0)}catch{return!1}}const at=n("view3d"),C=new Ke({antialias:!0});C.toneMapping=Tn,C.toneMappingExposure=1.22,C.setPixelRatio(Math.min(devicePixelRatio,2)),at.appendChild(C.domElement);const H=new Ze,A=new qe(42,1,.02,100),it=new Pn(16777215,2236962,.8),lt=new ye(16769725,1);lt.position.set(3,4.2,2.4);const xt=new ye(8229565,.3);xt.position.set(-2.6,1.4,-1.8);const vt=new Nn(16761470,0,14,2);H.add(vt),H.add(it,lt,xt);const un=new In(2,8,3753310,2305086);H.add(un);const Ct=new St(0,.5,0),Pe=new Qe(1,1,1);let bt=null;const hn=new qt({color:6003711,transparent:!0,opacity:.14,depthWrite:!1,side:tn}),At=new Dt(new en(1,1),hn);At.rotation.x=-Math.PI/2,H.add(At);const mn=new qt({color:16765286,transparent:!0,opacity:.1,depthWrite:!1,side:tn}),gt=new Dt(new en(1,1),mn);gt.rotation.y=Math.PI/2,gt.visible=!1,H.add(gt);const fn=new qt({color:6003711,transparent:!0,opacity:.28,depthWrite:!1}),ut=new Dt(new Qe(1,1,1),fn);ut.visible=!1,H.add(ut);function ne(){const t=nn.plain,e=On(t);H.background=new on(t.bg),it.color.setHex(e.hemiSky),it.groundColor.setHex(e.hemiGround),lt.color.setHex(e.sunCol),xt.color.setHex(e.fillCol),vt.color.setHex(e.heroCol),M==="live"?(it.intensity=e.hemiI,lt.intensity=e.sunI,xt.intensity=e.fillI,vt.intensity=e.heroI,we(e.actFill,e.actFillI)):M==="bright"?(it.intensity=.85,lt.intensity=1.05,xt.intensity=.3,vt.intensity=0,we(e.actFill,.1)):(it.intensity=.09,lt.intensity=.1,xt.intensity=.05,vt.intensity=0,we(0,0));const o=n("light");o.textContent=u(`tool.common.light.${M}`),o.classList.toggle("on",M!=="dark")}function Ne(t){const{geo:e,spots:o}=Hn(Yn(t),Wn(t),!1,Gn(t)),i=new Bn;i.add(new Dt(e,Rn()));for(const r of o){const c=new Dt(Pe,new qt({color:r.c}));c.position.set(r.x,r.y,r.z),c.scale.set(r.sx,r.sy,r.sz),i.add(c)}return i}function Ie(t){var e,o,i,r;for(const c of t.children)c.geometry!==Pe&&((o=(e=c.geometry)==null?void 0:e.dispose)==null||o.call(e)),(r=(i=c.material)==null?void 0:i.dispose)==null||r.call(i)}function j(t=!1){bt&&(H.remove(bt),Ie(bt)),bt=Ne(a),H.add(bt),yt(),t&&$e()}function yt(){const{w:t,d:e,h:o}=P(a),i=a.vox.s;At.scale.set(t*i+.06,e*i+.06,1),At.position.set(0,(h+.5)*i,0),gt.scale.set(e*i+.06,o*i+.06,1),gt.position.set(0,o*i/2,0),gt.visible=f}function $e(){const t=new an().setFromObject(bt),e=t.getCenter(new St),o=t.getSize(new St);Ct.copy(e),V=Math.max(o.x,o.y,o.z)*1.5+.35,vt.position.set(e.x+.4,1.9,t.max.z+1.3)}function oe(){const t=at.clientWidth,e=at.clientHeight;C.setSize(t,e),A.aspect=t/e,A.updateProjectionMatrix()}let jt=!1,ae=0,ie=0;C.domElement.addEventListener("pointerdown",t=>{if(t.pointerType!=="touch"){jt=!0,ae=t.clientX,ie=t.clientY;try{C.domElement.setPointerCapture(t.pointerId)}catch{}}}),C.domElement.addEventListener("pointermove",t=>{t.pointerType==="touch"||!jt||(F+=(t.clientX-ae)*.008,S=Math.min(1.35,Math.max(-.2,S+(t.clientY-ie)*.006)),ae=t.clientX,ie=t.clientY)}),C.domElement.addEventListener("pointerup",t=>{t.pointerType!=="touch"&&(jt=!1,b())}),C.domElement.addEventListener("wheel",t=>{t.preventDefault(),V=Math.min(20,Math.max(.3,V*(t.deltaY>0?1.08:.93)))},{passive:!1}),n("spin").addEventListener("click",t=>{I=!I,t.currentTarget.classList.toggle("on",I)}),n("light").addEventListener("click",()=>{M=M==="live"?"bright":M==="bright"?"dark":"live",ne(),b()});const U=n("slice"),xn=U.getContext("2d"),Oe=n("stage");let y=28,Y=null;function E(){const{w:t,d:e,h:o}=P(a),i=Oe.clientWidth-48,r=Oe.clientHeight-48;y=Math.max(8,Math.min(46,Math.floor(Math.min(i/t,r/e))));const c=t*y+1,v=e*y+1,N=Math.min(devicePixelRatio,2);U.width=c*N,U.height=v*N,U.style.width=`${c}px`,U.style.height=`${v}px`;const x=xn;if(x.setTransform(N,0,0,N,0,0),x.clearRect(0,0,c,v),x.fillStyle="#0d1117",x.fillRect(0,0,c,v),k&&h>0){x.globalAlpha=.26;for(let w=0;w<e;w++)for(let R=0;R<t;R++){const et=pt(a,R,h-1,w);et&&(x.fillStyle=a.vox.pal[et]??"#888",x.fillRect(R*y+1,w*y+1,y-1,y-1))}x.globalAlpha=1}for(let w=0;w<e;w++)for(let R=0;R<t;R++){const et=pt(a,R,h,w);et&&(x.fillStyle=a.vox.pal[et]??"#f0f",x.fillRect(R*y+1,w*y+1,y-1,y-1),ce().has((a.vox.pal[et]??"").toLowerCase())&&(x.fillStyle="rgba(255,244,200,0.9)",x.font=`${Math.max(8,y*.45)}px sans-serif`,x.fillText("✦",R*y+y*.28,w*y+y*.72)))}x.strokeStyle="rgba(90,110,150,0.28)",x.lineWidth=1;for(let w=0;w<=t;w++)x.beginPath(),x.moveTo(w*y+.5,0),x.lineTo(w*y+.5,v),x.stroke();for(let w=0;w<=e;w++)x.beginPath(),x.moveTo(0,w*y+.5),x.lineTo(c,w*y+.5),x.stroke();if(f&&(x.strokeStyle="rgba(255,209,102,0.6)",x.setLineDash([5,4]),x.beginPath(),x.moveTo(t/2*y+.5,0),x.lineTo(t/2*y+.5,v),x.stroke(),x.setLineDash([])),Y&&(x.strokeStyle=p==="erase"?"#e06a5a":"#5b9bff",x.lineWidth=2,x.strokeRect(Y.x*y+1.5,Y.z*y+1.5,y-2,y-2),f)){const w=P(a).w-1-Y.x;w!==Y.x&&(x.globalAlpha=.5,x.strokeRect(w*y+1.5,Y.z*y+1.5,y-2,y-2),x.globalAlpha=1)}n("layerlbl").textContent=u("tool.sculpt.layerLbl",{n:h+1,h:o})}function Fe(t){const e=U.getBoundingClientRect(),o=Math.floor((t.clientX-e.left)/y),i=Math.floor((t.clientY-e.top)/y),{w:r,d:c}=P(a);return o>=0&&i>=0&&o<r&&i<c?{x:o,z:i}:null}const le=n("msg"),Ut=matchMedia("(max-width: 900px)");let se=0;function O(t,e=!1){le.textContent=t,le.style.color=e?"#ffd166":"#7fd17f",clearTimeout(se);const o=n("mtoast");Ut.matches&&(o.textContent=t,o.style.display="block",o.style.borderColor=e?"#ffd166":"#2b3547"),se=window.setTimeout(()=>{le.textContent="",o.style.display="none"},4500)}function Gt(t){n("guide").style.display=t?"block":"none",n("mlegend").style.display=t&&Ut.matches?"block":"none"}n("help").addEventListener("click",()=>Gt(!0));function K(t){t&&(E(),j(),Gt(!1))}function D(){T.commit(a)&&(Wt(),kt(),G(),b())}function Z(t,e){a=t,Ee(a),h=Math.min(h,P(a).h-1),De(),Mt(),E(),j(!0),G(),D(),e&&O(e)}function De(){p!=="erase"&&!(p in a.vox.pal)&&(p=Object.keys(a.vox.pal)[0]??"erase")}function Wt(){n("undo").disabled=!T.canUndo(),n("redo").disabled=!T.canRedo(),n("mUndo").disabled=!T.canUndo(),n("mRedo").disabled=!T.canRedo()}function Tt(){const t=T.undo();t&&(a=t,h=Math.min(h,P(a).h-1),Mt(),E(),j(),G(),Wt(),kt(),b(),O(u("tool.common.undone")))}function wt(){const t=T.redo();t&&(a=t,h=Math.min(h,P(a).h-1),Mt(),E(),j(),G(),Wt(),kt(),b(),O(u("tool.common.redone")))}n("undo").addEventListener("click",Tt),n("redo").addEventListener("click",wt);let ht="none",st=null;function He(t,e,o){K(Vt(a,e,h,o,t||p==="erase"?null:p,f))}function re(t,e){if(st&&(st.x!==e.x||st.z!==e.z)){const{x:o,z:i}=st,r=Math.max(Math.abs(e.x-o),Math.abs(e.z-i));for(let c=1;c<=r;c++)He(t,Math.round(o+(e.x-o)*(c/r)),Math.round(i+(e.z-i)*(c/r)))}else He(t,e.x,e.z);st=e}U.addEventListener("pointerdown",t=>{try{U.setPointerCapture(t.pointerId)}catch{}const e=Fe(t);if(e){if(t.button===2){ht="erase",st=null,re(!0,e);return}if(t.button===0){if(g==="fill"){K(Me(a,h,e.x,e.z,p==="erase"?null:p,f)),D();return}ht="paint",st=null,re(p==="erase",e)}}}),U.addEventListener("pointermove",t=>{const e=Fe(t);Y=e,Pt(e),(ht==="paint"||ht==="erase")&&e?re(ht==="erase"||p==="erase",e):E()}),U.addEventListener("pointerup",()=>{ht!=="none"&&D(),ht="none",st=null}),U.addEventListener("pointerleave",()=>{Y=null,Pt(null),E()}),U.addEventListener("contextmenu",t=>t.preventDefault());function Pt(t){const e=n("hoverhint");if(!t){e.textContent=u("tool.sculpt.hoverIdle");return}const o=pt(a,t.x,h,t.z),i=o?`${a.vox.pal[o]}（'${o}'）`:u("tool.sculpt.empty"),r=p==="erase"?u("tool.sculpt.verb.erase"):g==="fill"?u("tool.sculpt.verb.fill",{name:a.vox.pal[p]??p}):u("tool.sculpt.verb.carve",{name:a.vox.pal[p]??p});e.textContent=u("tool.sculpt.hoverLine",{x:t.x,n:h+1,z:t.z,what:i,verb:r,mirror:f?u("tool.sculpt.mirrorSuffix"):""})}function ce(){return new Set(a.glow.map(t=>t.toLowerCase()))}function G(){const t=n("pal");t.innerHTML="";const e=Se(a),o=ce();for(const[c,v]of Object.entries(a.vox.pal)){const N=document.createElement("div");N.className="chip"+(p===c?" on":""),N.style.background=v,N.title=u("tool.sculpt.chipTitle",{hex:v,ch:c})+(o.has(v.toLowerCase())?u("tool.sculpt.glowMark"):""),N.innerHTML=`<span class="ch">${nt(c)}</span><span class="n">${e.get(c)??0}</span>${o.has(v.toLowerCase())?'<span class="glow">✦</span>':""}`,N.addEventListener("click",()=>q(c)),t.appendChild(N)}const i=document.createElement("div");i.className="chip erase"+(p==="erase"?" on":""),i.title=u("tool.sculpt.eraserTitle"),i.textContent="🧽",i.addEventListener("click",()=>q("erase")),t.appendChild(i);const r=document.createElement("div");r.className="chip erase",r.title=u("tool.sculpt.addColor"),r.textContent="＋",r.addEventListener("click",()=>{const c=rn(a,vn());if(!c){O(u("tool.sculpt.palExhausted"),!0);return}q(c),D(),G(),O(u("tool.sculpt.colorAdded"))}),t.appendChild(r),bn(),rt()}function vn(){const t=Math.random()*360,e=.45+Math.random()*.3,o=.45+Math.random()*.2,i=r=>{const c=(r+t/30)%12,v=e*Math.min(o,1-o);return Math.round(255*(o-v*Math.max(-1,Math.min(c-3,9-c,1))))};return"#"+[i(0),i(8),i(4)].map(r=>r.toString(16).padStart(2,"0")).join("")}function bn(){const t=n("palopts");if(p==="erase"){t.innerHTML=`<span class="sub">${nt(u("tool.sculpt.palOptErase"))}</span>`;return}const e=a.vox.pal[p];if(!e){t.innerHTML="";return}t.innerHTML=`<span>${nt(u("tool.sculpt.selected"))} '<b>${nt(p)}</b>'</span>`;const o=document.createElement("input");o.type="color",o.value=e,o.addEventListener("input",()=>{cn(a,p,o.value)&&(E(),j())}),o.addEventListener("change",()=>{D(),G()});const i=document.createElement("label");i.className="tog";const r=document.createElement("input");r.type="checkbox",r.checked=ce().has(e.toLowerCase()),r.addEventListener("change",()=>{dn(a,a.vox.pal[p],r.checked),D(),G(),E(),j(),O(r.checked?u("tool.sculpt.glowOn"):u("tool.sculpt.glowOff"))}),i.append(r,document.createTextNode(" "+u("tool.sculpt.glowLabel")));const c=Se(a).get(p)??0,v=document.createElement("span");v.className="sub",v.textContent=u("tool.sculpt.usedVox",{n:c}),t.append(o,i,v)}function q(t){p=t,G(),Pt(Y),E(),b()}function Nt(t){g=t,rt(),b()}function rt(){n("shBrush").classList.toggle("on",g==="brush"),n("shFill").classList.toggle("on",g==="fill");const t=n("nowswatch");p==="erase"?(t.style.background="repeating-linear-gradient(45deg,#232c3e,#232c3e 4px,#3a465e 4px,#3a465e 8px)",n("nowname").textContent=u("tool.sculpt.eraser")):(t.style.background=a.vox.pal[p]??"#f0f",n("nowname").textContent=`${a.vox.pal[p]??"?"}（'${p}'）`),n("nowshape").textContent=`（${u(`tool.common.shape.${g}`)}）`,n("mirror").checked=f,n("onion").checked=k,ct()}function ct(){for(const[v,N]of[["tCarve","carve"],["tPaint","paint"],["tErase","erase"],["tBox","box"],["tFill","fill"]])n(v).classList.toggle("on",L===N);const t=n("hColorSw"),e=Xt();t.style.background=e?a.vox.pal[e]??"#f0f":"#232c3e",n("hColorName").textContent=e?`'${e}'`:"—",n("mMirror").classList.toggle("on",f),n("lLock").classList.toggle("on",z);const{w:o,d:i,h:r}=P(a);n("mInfo").textContent=`${u("tool.sculpt.layerLbl",{n:h+1,h:r})}・${o}×${i}×${r}・${ke(a)}v`;const c=n("lSlider");c.max=String(r),c.value=String(h+1),n("lNum").textContent=`${h+1}/${r}`}n("shBrush").addEventListener("click",()=>Nt("brush")),n("shFill").addEventListener("click",()=>Nt("fill")),n("mirror").addEventListener("change",t=>{f=t.target.checked,yt(),ct(),E(),b()}),n("onion").addEventListener("change",t=>{k=t.target.checked,E(),b()});function dt(t){const{h:e}=P(a);h=Math.max(0,Math.min(e-1,t)),E(),yt(),Pt(Y),ct(),b()}n("layUp").addEventListener("click",()=>dt(h+1)),n("layDn").addEventListener("click",()=>dt(h-1));function Mt(){n("mid").value=a.id,n("mname").value=a.name,n("ms").value=String(a.vox.s);const{w:t,d:e,h:o}=P(a);n("mw").value=String(t),n("md").value=String(e),n("mh").value=String(o)}function Yt(){a.id=n("mid").value.trim()||a.id,a.name=n("mname").value.trim()||a.name,a.vox.s=Math.max(.02,Math.min(.3,Number(n("ms").value)||a.vox.s))}for(const t of["mid","mname"])n(t).addEventListener("change",()=>{Yt(),D(),kt()});n("ms").addEventListener("change",()=>{Yt(),E(),j(!0),D()}),n("applySize").addEventListener("click",()=>{const t=Math.max(1,Math.min(64,Number(n("mw").value)||1)),e=Math.max(1,Math.min(64,Number(n("md").value)||1)),o=Math.max(1,Math.min(64,Number(n("mh").value)||1)),{dropped:i}=sn(a,t,e,o);h=Math.min(h,o-1),Mt(),E(),j(!0),D(),O(i?u("tool.sculpt.resizedDrop",{w:t,d:e,h:o,n:i}):u("tool.sculpt.resized",{w:t,d:e,h:o}),i>0)}),n("newm").addEventListener("click",()=>{const{w:t,d:e,h:o}=P(a),i=Math.floor(Math.random()*900)+100;f=!0,Z(Le(`model-${i}`,u("tool.sculpt.newName",{n:i}),t,e,o,J()),u("tool.sculpt.newSaid"))});function Be(){const t=n("typelist");t.innerHTML="";for(const[e,o]of B){const i=document.createElement("option");i.value=e,i.label=`${ft(o)}${Bt[e]?u("tool.sculpt.srcCode"):Dn[e]?u("tool.sculpt.srcLarge"):u("tool.sculpt.srcProc")}`,t.appendChild(i)}}function kt(){Yt();const t=n("lint"),e=[],o=Un.safeParse({...a,updatedAt:J()});if(!o.success)for(const x of o.error.issues.slice(0,6))e.push({level:"err",msg:x.message});const i=ke(a);e.push({level:i>0?"ok":"err",msg:u("tool.sculpt.lintCount",{n:i,c:Object.keys(a.vox.pal).length,g:a.glow.length})}),B.has(a.id)?e.push({level:"ok",msg:u("tool.sculpt.lintMatch",{id:a.id,name:ft(B.get(a.id))})}):e.push({level:"warn",msg:u("tool.sculpt.lintNoMatch",{id:a.id})}),t.innerHTML=e.map(x=>`<div class="s-${x.level}">${x.level==="ok"?"✓":x.level==="warn"?"⚠":"✗"} ${nt(x.msg)}</div>`).join("");const r=n("save");r.disabled=!o.success,n("savenote").textContent=o.success?u("tool.common.savenote"):u("tool.sculpt.lintFix");const c=e.filter(x=>x.level==="err").length,v=e.filter(x=>x.level==="warn").length,N=n("mLint");N.textContent=c?`✗ ${c}`:v?`⚠ ${v}`:"✓",N.className="mchip "+(c?"err":v?"warn":"ok"),n("mSave").disabled=!o.success,ct()}const gn=t=>{var i;const e=(i=t.target)==null?void 0:i.tagName;if(e==="INPUT"||e==="SELECT"||e==="TEXTAREA")return;if((t.metaKey||t.ctrlKey)&&t.key.toLowerCase()==="z"){t.preventDefault(),t.shiftKey?wt():Tt();return}if((t.ctrlKey||t.metaKey)&&t.key.toLowerCase()==="y"){t.preventDefault(),wt();return}const o=t.key.toLowerCase();if(o==="b")Nt("brush");else if(o==="f")Nt("fill");else if(o==="e")q("erase");else if(o==="m")f=!f,yt(),rt(),E(),b();else if(o==="o")k=!k,rt(),E(),b();else if(t.key==="[")dt(h-1);else if(t.key==="]")dt(h+1);else if(o==="escape")Gt(!1);else if(/^[1-9]$/.test(t.key)){const c=Object.keys(a.vox.pal)[Number(t.key)-1];c&&q(c)}};l.own(window,"keydown",gn);let Re=null;function Ve(){return Yt(),mt({...a,updatedAt:J()})}n("save").addEventListener("click",()=>{de()});async function de(){try{const t=Ve(),e=await m(d("/api/sculpt/proposal"),{model:t,label:t.name,createdBy:"user"});O(u("tool.common.savedFuture",{id:e.id.slice(0,8)}))}catch(t){O(u("tool.common.saveFail",{msg:t.message}),!0)}}async function Ae(t){try{const e=await m(d("/api/sculpt/promote"),{proposalId:t});O(u("tool.sculpt.promoted",{note:e.promotedNote??u("tool.common.adoptedNote")}))}catch(e){O(u("tool.common.promoteFail",{msg:e.message}),!0)}}async function je(t){try{await m(d("/api/sculpt/archive"),{proposalId:t}),O(u("tool.common.archived"))}catch(e){O(u("tool.common.archiveFail",{msg:e.message}),!0)}}const It=new Ke({antialias:!0,preserveDrawingBuffer:!0});It.setSize(96,96);const zt=new Ze,pe=new qe(40,1,.02,50);zt.add(new $n(16777215,.85));const Ue=new ye(16769725,1);Ue.position.set(3,6,4),zt.add(Ue);const ue=new Map;function yn(t){const e=`${t.id}:${t.updatedAt}`;if(ue.has(e))return ue.get(e);zt.background=new on(nn.plain.bg);const o=Ne(t);zt.add(o);const i=new an().setFromObject(o),r=i.getCenter(new St),c=i.getSize(new St),v=Math.max(c.x,c.y,c.z)*.62+.1;pe.position.set(r.x+v*1.35,r.y+v*1.15,r.z+v*1.85),pe.lookAt(r),It.render(zt,pe);const N=It.domElement.toDataURL();return zt.remove(o),Ie(o),ue.set(e,N),N}let he=!1;async function $t(){try{const[t,e]=await Promise.all([fetch(d("/api/sculpt/truth")).then(o=>o.json()),fetch(d("/api/sculpt/proposals")).then(o=>o.json())]);he=!0,wn(e.proposals??[]),Ge(t.models??[]),n("live").innerHTML="<b>· live</b>"}catch{he=!1,n("live").textContent=u("tool.common.devOff");const t=[];for(const e of Fn())try{t.push(Rt(e.id,e.name,e.vox,e.glow,J()))}catch{}Ge(t),n("proposals").innerHTML=`<div class="live">${nt(u("tool.sculpt.devOnlyLoop"))}</div>`}Mn()}function wn(t){Kn(n("proposals"),t,{selectedId:Re,onView:e=>{kn(e)},onPromote:e=>{Ae(e)},onArchive:e=>{je(e)}})}function Ge(t){Zn(n("truth"),t.map(e=>{const{w:o,d:i,h:r}=P(e);let c=null;try{c=yn(e)}catch{}return{id:e.id,name:ft(e.name),meta:`${o}×${i}×${r}・s=${e.vox.s}・${B.has(e.id)?u("tool.sculpt.overriding",{name:ft(B.get(e.id))}):u("tool.sculpt.noMatch")}`,thumb:c,onOpen:()=>{Z(mt(e),u("tool.common.openedTruth",{name:e.name}))},onDup:()=>{const v=mt({...e,id:`${e.id}-2`,name:u("tool.common.copyName",{name:e.name}),updatedAt:J()});Z(v,u("tool.common.duplicated",{name:e.name}))},onCopyWorld:()=>{(async()=>{try{const v=await Qn("/api/sculpt","model",mt(e),l.worlds(),m);v&&O(u("tool.world.copied",{world:v}))}catch(v){O(u("tool.world.copyFail",{msg:v.message}),!0)}})()}}}),u("tool.sculpt.truthEmpty"))}function Mn(){const t=n("codelib");t.innerHTML="";for(const[o,i]of Object.entries(Bt)){const r=document.createElement("div");r.className="card",r.innerHTML=`<div><b>${nt(ft(B.get(o)??o))}</b> <span class="live">MON_VOX['${nt(o)}']</span></div>`;const c=document.createElement("div");c.className="row";const v=document.createElement("button");v.textContent=u("tool.sculpt.importEdit"),v.addEventListener("click",()=>{Z(Rt(o,B.get(o)??o,i,Qt[o]??[],J()),u("tool.sculpt.imported",{name:ft(B.get(o)??o)}))}),c.appendChild(v),r.appendChild(c),t.appendChild(r)}const e=document.createElement("div");e.className="card",e.innerHTML=`<div class="live">${nt(u("tool.sculpt.otherNote"))}</div>`,t.appendChild(e)}async function kn(t){const e=await fetch(d(`/api/sculpt/proposal?id=${encodeURIComponent(t)}`)).then(i=>i.json());Re=t;const o=(e.models??[])[e.models.length-1];o&&Z(mt(o),u("tool.common.viewingFuture",{id:t.slice(0,8)})),$t()}const zn=l.events.on("sculpt",()=>{$t()});function Ln(){const t=Math.round(F/(Math.PI/4))*(Math.PI/4);Math.abs(F-t)<5*Math.PI/180&&(F=t)}const me=new An;function Ot(t,e){const o=C.domElement.getBoundingClientRect();me.setFromCamera(new jn((t-o.left)/o.width*2-1,-((e-o.top)/o.height*2-1)),A);const i=me.ray.origin,r=me.ray.direction;return{origin:{x:i.x,y:i.y,z:i.z},dir:{x:r.x,y:r.y,z:r.z}}}const fe=(t,e,o)=>pt(a,t,e,o)!=null;function Xt(){return L==="erase"?null:p!=="erase"&&p in a.vox.pal?p:Object.keys(a.vox.pal)[0]??null}let tt="none",Lt=null,xe=!1,Q=null,X=null,_t=new Set,Jt=null,_=null,W=null;const Et=t=>`${t.x},${t.y},${t.z}`;function ve(){tt="none",Lt=null,xe=!1,Q=null,X=null,Jt=null,_=W=null,_t=new Set,ut.visible=!1}function We(){if(!_||!W){ut.visible=!1;return}const t=P(a),e=a.vox.s,o={x:Math.min(_.x,W.x),y:Math.min(_.y,W.y),z:Math.min(_.z,W.z)},i={x:Math.max(_.x,W.x),y:Math.max(_.y,W.y),z:Math.max(_.z,W.z)},r=ze(o,t,e),c=ze(i,t,e);ut.visible=!0,ut.scale.set((i.x-o.x+1)*e+.004,(i.y-o.y+1)*e+.004,(i.z-o.z+1)*e+.004),ut.position.set((r.x+c.x)/2,(r.y+c.y)/2,(r.z+c.z)/2)}function Kt(t,e){let o=!1;for(const i of t)o=Vt(a,i.x,i.y,i.z,e,f)||o,_t.add(Et(i));return o}_n(C.domElement,{strokeStart(t,e){ve();const o=Xt();if(L!=="erase"&&o==null)return;const{origin:i,dir:r}=Ot(t,e),c=oo(i,r,L,P(a),a.vox.s,fe,h,z);if(c){if(c.plane&&(c.plane.k<0||c.plane.k>=te(P(a),c.plane.axis))){O(u("tool.sculpt.atEdge"),!0);return}if(Jt=JSON.stringify(a),Lt=c.plane,xe=c.surface,Q=c.cell,X={x:t,y:e},L==="fill"){tt="fill",_t.add(Et(c.cell)),K(Me(a,c.cell.y,c.cell.x,c.cell.z,o,f));return}if(L==="box"){tt="box",_=W=c.cell,We();return}tt="stroke",K(Kt([c.cell],o))}},strokeMove(t,e){const o=P(a),i=a.vox.s;if(tt==="box"){if(!Lt)return;const x=Ot(t,e),w=ee(x.origin,x.dir,Lt,o,i);w&&(W=w,We());return}if(tt!=="stroke")return;const r=Xt();if(xe){if(!X)return;const x=Math.max(1,Math.ceil(Math.hypot(t-X.x,e-X.y)/8));let w=!1;for(let R=1;R<=x;R++){const et=Ot(X.x+(t-X.x)*(R/x),X.y+(e-X.y)*(R/x)),Zt=Te(et.origin,et.dir,o,i,fe);Zt&&(!Q||Et(Zt.cell)!==Et(Q))&&(w=Kt([Zt.cell],r)||w,Q=Zt.cell)}X={x:t,y:e},K(w);return}if(!Lt)return;const c=Ot(t,e),v=ee(c.origin,c.dir,Lt,o,i);if(!v||Q&&Et(v)===Et(Q))return;const N=Kt(Q?ao(Q,v):[v],r);Q=v,X={x:t,y:e},K(N)},strokeEnd(){tt==="box"&&_&&W&&K(Kt(io(_,W),Xt())),tt!=="none"&&(D(),Ht()),ve()},strokeCancel(){tt!=="none"&&Jt&&(a=JSON.parse(Jt),E(),j()),ve()},orbit(t,e){F+=t*.008,S=Math.min(1.35,Math.max(-.2,S+e*.006))},orbitEnd(){Ln(),b()},pinch(t){V=Math.min(20,Math.max(.3,V/t))},undoTap(){Tt(),Ht()},redoTap(){wt(),Ht()},longPress(t,e){const o=Ot(t,e),i=Te(o.origin,o.dir,P(a),a.vox.s,fe),r=i?pt(a,i.cell.x,i.cell.y,i.cell.z):null;r&&(L==="erase"&&(L="carve"),q(r),O(u("tool.sculpt.picked",{hex:a.vox.pal[r]??"?",ch:r})),Ht())},strokeSize:()=>_t.size});const Ft=Jn(["sheetSide","sheetLoop"]);function Ye(){Ut.matches?(n("sheetSideBody").append(n("topbar"),n("side")),n("sheetLoopBody").append(n("libwrap")),n("mstage").appendChild(n("view3d"))):(Ft.closeAll(),n("app").insertBefore(n("side"),n("stagewrap")),n("stagewrap").insertBefore(n("topbar"),n("mchips")),n("loop").append(n("view3d"),n("libwrap"))),oe(),E()}n("mMenu").addEventListener("click",()=>Ft.open("sheetSide")),n("mShelf").addEventListener("click",()=>Ft.open("sheetLoop")),n("mLint").addEventListener("click",()=>Ft.open("sheetSide")),n("hColor").addEventListener("click",()=>Ft.open("sheetSide")),n("mUndo").addEventListener("click",Tt),n("mRedo").addEventListener("click",wt),n("mSave").addEventListener("click",()=>{Ht(),de()}),n("mFit").addEventListener("click",()=>{$e(),b()}),n("mVF").addEventListener("click",()=>{F=0,S=.15,b()}),n("mVS").addEventListener("click",()=>{F=Math.PI/2,S=.15,b()}),n("mVT").addEventListener("click",()=>{F=Math.round(F/(Math.PI/2))*(Math.PI/2),S=1.35,b()}),n("mMirror").addEventListener("click",()=>{f=!f,yt(),rt(),E(),b()});for(const[t,e]of[["tCarve","carve"],["tPaint","paint"],["tErase","erase"],["tBox","box"],["tFill","fill"]])n(t).addEventListener("click",()=>{L=e,ct(),b()});n("lUp").addEventListener("click",()=>dt(h+1)),n("lDn").addEventListener("click",()=>dt(h-1)),n("lSlider").addEventListener("input",t=>dt(Number(t.target.value)-1)),n("lLock").addEventListener("click",()=>{z=!z,ct(),b()}),l.own(Ut,"change",Ye);function be(){I&&!jt&&(F+=.006),A.position.set(Ct.x+V*Math.sin(F)*Math.cos(S),Ct.y+V*Math.sin(S),Ct.z+V*Math.cos(F)*Math.cos(S)),A.lookAt(Ct),C.render(H,A)}let Xe=0,_e=!1;function Je(){_e||(Xe=requestAnimationFrame(Je),be())}const En=()=>{oe(),E()};l.own(window,"resize",En),window.cdSculpt={paint:(t,e,o)=>(o&&q(o),K(Vt(a,t,h,e,p==="erase"?null:p,f)),D(),pt(a,t,h,e)),erase:(t,e)=>(K(Vt(a,t,h,e,null,f)),D(),pt(a,t,h,e)),fill:(t,e,o)=>{o&&q(o),K(Me(a,h,t,e,p==="erase"?null:p,f)),D()},tool:t=>q(t),shapeMode:t=>Nt(t),setLayer:dt,layerNow:()=>h,mirrorOn:t=>{f=t,yt(),rt(),E(),b()},addColor:t=>{const e=rn(a,t);return e&&(q(e),D(),G()),e},recolor:(t,e)=>{const o=cn(a,t,e);return o&&(E(),j(),D(),G()),o},glowHex:(t,e)=>{const o=dn(a,t,e);return o&&(D(),G(),j()),o},resize:(t,e,o)=>{const i=sn(a,t,e,o);return h=Math.min(h,o-1),Mt(),E(),j(!0),D(),i},newModel:(t,e,o,i,r)=>Z(Le(t,e,o,i,r,J())),importCode:t=>{const e=Bt[t];return e?(Z(Rt(t,B.get(t)??t,e,Qt[t]??[],J())),!0):!1},openTruthJson:t=>Z(mt(t)),model:()=>Ve(),draft:()=>a,dims:()=>P(a),count:()=>ke(a),undo:Tt,redo:wt,save:()=>de(),promote:t=>Ae(t),archive:t=>je(t),refresh:()=>$t(),state:()=>({brush:p,shape:g,layer:h,mirror:f,onion:k,lightMode:M,devLive:he,ttool:L,layerLock:z,canUndo:T.canUndo(),canRedo:T.canRedo(),guide:n("guide").style.display!=="none"}),view:(t,e,o)=>{F=t,S=e,o!=null&&(V=o)},step:()=>be(),draws:()=>C.info.render.calls,screenOf:(t,e)=>{const o=U.getBoundingClientRect();return{cx:o.left+(t+.5)*y,cy:o.top+(e+.5)*y}},mtool:t=>{L=t,ct()},lockLayer:t=>{z=t,ct()},vox:(t,e,o)=>pt(a,t,e,o),screenOfVox:(t,e,o)=>{be();const i=ze({x:t,y:e,z:o},P(a),a.vox.s),r=new St(i.x,i.y,i.z).project(A),c=C.domElement.getBoundingClientRect();return{cx:c.left+(r.x+1)/2*c.width,cy:c.top+(1-r.y)/2*c.height}}};async function Sn(t){try{const i=((await fetch(d("/api/sculpt/truth")).then(r=>r.json())).models??[]).find(r=>r.id===t);if(i){Z(mt(i),u("tool.common.openedTruth",{name:i.name}));return}}catch{}const e=Bt[t];e?Z(Rt(t,B.get(t)??t,e,Qt[t]??[],J()),u("tool.sculpt.imported",{name:ft(B.get(t)??t)})):O(u("tool.studio.deepMiss",{id:t}),!0)}const Cn=to(()=>{G(),rt(),kt(),E(),Be(),ne(),Pt(Y),$t()}),ge=$();return Ee(a),De(),T.reset(a),Be(),Mt(),G(),rt(),Wt(),kt(),ne(),Ye(),oe(),E(),j(!ge),Gt(!ge),ge&&O(u("tool.common.restored")),$t(),Je(),{applyParams(t){const e=t.get("type");e&&Sn(e)},dispose(){_e=!0,cancelAnimationFrame(Xe),clearTimeout(se),zn(),Cn(),b();try{C.dispose(),C.forceContextLoss(),C.domElement.remove()}catch{}try{It.dispose(),It.forceContextLoss()}catch{}delete window.cdSculpt}}}};export{ko as sculptPanel};
