import{p as T,w as P,f as S}from"./creator-auth-C9Tw5ccD.js";import{o as $,P as H}from"./studio-CPz-TNtB.js";import{b as A,t as o}from"./i18n-CUpE17Zf.js";import"./preload-helper-Dp1pzeXC.js";const D=`<!-- ============================================================
...
`,q={id:"worlds",async mount(g,w){g.innerHTML=D,A(g);const r=a=>g.querySelector(`#${a}`);function i(a,t=!1){const d=r("say");d.textContent=a,d.className=t?"err":""}let h="masaru",l=[];function L(a,t){if(t===w.world()){w.navigate(a);return}const d=w.standalone?new URL("studio.html",window.location.href):new URL(window.location.href);d.search="",d.searchParams.set("panel",a),d.searchParams.set("world",t),window.location.href=d.toString()}function y(){const a=r("worlds");if(a.innerHTML="",!l.length){const e=document.createElement("div");e.className="live",e.textContent=o("worlds.none"),a.appendChild(e);return}for(const e of l){const p=document.createElement("div");p.className="card";const N=document.createElement("div"),z=document.createElement("b");z.textContent=e.title,N.appendChild(z);const v=document.createElement("div");v.className="key",v.textContent=e.key;const b=document.createElement("div");b.className="tagrow";const k=document.createElement("span");k.className="tag"+(e.publishedAt?" pub":""),k.textContent=e.publishedAt?o("worlds.statusPublished"):o("worlds.statusDraft"),b.appendChild(k);const F=e.forkedFrom;if(F){const n=document.createElement("span");n.className="tag",n.textContent=o("worlds.forkedFrom",{world:F}),b.appendChild(n)}p.append(N,v,b);const C=document.createElement("div");C.className="row";for(const n of H){if(n.id==="worlds")continue;const u=document.createElement("button");u.type="button",u.className="btn",u.textContent=`${n.icon} ${o(n.titleKey)}`,u.addEventListener("click",()=>L(n.id,e.key)),C.appendChild(u)}p.appendChild(C);const E=document.createElement("div");E.className="row";const c=document.createElement("button");c.type="button",c.className="on",c.textContent=o("worlds.publish"),c.addEventListener("click",()=>{(async()=>{try{const n=await T("/api/worlds/publish",{world:e.key});i(o("worlds.published",{path:n.packPath})),await f()}catch(n){i(o("worlds.publishFail",{msg:n.message}),!0)}})()});const x=document.createElement("button");x.type="button",x.textContent=o("tool.common.dup"),x.addEventListener("click",()=>{r("fldFrom").value=e.key,r("fldId").focus();const n=r("wapp");n.scrollTo({top:n.scrollHeight,behavior:"smooth"})});const m=document.createElement("a");m.className="btn",m.textContent=o("worlds.play"),m.href=`dungeon.html?pack=${encodeURIComponent(`packs/${e.packName}.json`)}`,m.target="_blank",E.append(c,x,m),p.appendChild(E),a.appendChild(p)}const t=r("fldFrom"),d=t.value;t.innerHTML="";const s=document.createElement("option");s.value="",s.textContent=o("worlds.fromEmpty"),t.appendChild(s);for(const e of l){const p=document.createElement("option");p.value=e.key,p.textContent=`${e.title}（${e.key}）`,t.appendChild(p)}[...t.options].some(e=>e.value===d)&&(t.value=d)}async function f(){const a=await S();if(!a){i(o("worlds.loadFail",{msg:"API unreachable"}),!0),l=[],y();return}h=a.accountId,l=a.worlds,r("acct").textContent=h,y()}r("createForm").addEventListener("submit",a=>{a.preventDefault();const t=r("fldId").value.trim(),d=r("fldTitle").value.trim()||t,s=r("fldFrom").value||void 0;(async()=>{try{const e=await T("/api/worlds",{id:t,title:d,...s?{from:s}:{}});i(o("worlds.created",{key:e.key})),r("fldId").value="",r("fldTitle").value="",await f()}catch(e){i(o("worlds.createFail",{msg:e.message}),!0)}})()}),window.cdWorlds={reload:()=>f(),worlds:()=>l,account:()=>h,api:a=>P(a),open:(a,t)=>L(a,t??w.world())};const I=$(()=>{y()});return f(),{applyParams(){},dispose(){I(),delete window.cdWorlds}}}};export{q as worldsPanel};
